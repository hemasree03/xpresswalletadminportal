	...
	public ResponseDTO fetchData(String refrenceno,String makerid) {
		...
		try {
			...
			branchJSONArray = new JSONArray();
			 ...
			 }else if((refrenceno.split("-")[0]).equalsIgnoreCase("PRODUCT") && (refrenceno.split("-")[1]).equalsIgnoreCase("BILLER")){
				 branchQry="select distinct CATGRP_CODE from PAYBILLCATE_GRP_TBL"; 
				 branchPstmt=connection.prepareStatement(branchQry);
			 ...
			 }else if((refrenceno.split("-")[1]).equalsIgnoreCase("SUPERAGENT")){
				 branchQry="select  SUPERAGENT_NAME from SUPER_AGENT ";
				 branchPstmt=connection.prepareStatement(branchQry);
				 JSONObject json2 = new JSONObject();
					json2.put("accountno","EXPRESS");
					branchJSONArray.add(json2);
					JSONObject json3 = new JSONObject();
					   json3.put("accountno","ALL");
