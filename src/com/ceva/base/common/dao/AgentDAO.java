...
public class AgentDAO {
			...
			viewDataMap = new HashMap<String, Object>();
			 ...
			 String Actiondesc="";
				  ...
				  servicePstmt = connection.prepareStatement("select ACM.ACCOUNT_NO,ACM.FIRST_NAME,ACM.MIDDLE_NAME,ACM.LAST_NAME,ACM.ACC_BRANCH,to_char(ACM.DOB,'dd-mm-yyyy'),ACM.EMAILID,ACM.GENDER,ACM.SUPER_ADM,ACM.W_PRD_CODE,MCI.MOBILE_NUMBER,"
				  		+ "MCI.ADDRESS,MCI.ID_TYPE,MCI.ID_DETAILS,MCI.NATIONALITY,MCI.RL_LGA,MCI.R_STATE,MCI.COUNTRY,decode(ACM.STATUS,'B','Block','F','Inactive','A','Active','L','Deactivation','Active'),ACM.ID,NVL(ACM.USER_ID,' '),WAD.ACCT_NO,to_char(ACM.DATE_CREATED,'dd-mm-yyyy hh:mi:ss'),W_PRD_CODE,W_PRD_DESC,AGENCY_TYPE,"
				  		+ "decode(ACM.USSD_STATUS,'A','Active','Deactivated'),decode(ACM.MOBILE_STATUS,'A','Active','Deactivated'),decode(ACM.POS_STATUS,'A','Active','Deactivated') from AGENT_CUSTOMER_MASTER ACM,AGENT_CONTACT_INFO MCI,WALLET_ACCT_DATA WAD where ACM.ID=MCI.CUST_ID and MCI.CUST_ID=WAD.CUST_ID and WAD.CUST_TYPE='AGENT' and ACM.ID='"+custid+"'");
				  serviceRS = servicePstmt.executeQuery();
					
					 while(serviceRS.next())
						{
						json.put("refno",custid);
						...
						json.put("ussd_status",serviceRS.getString(27));
						json.put("mobile_status",serviceRS.getString(28));
						json.put("pos_status",serviceRS.getString(29));
		...
		try {
			...
			JSONObject json = new JSONObject();
			 ...
			 String Actiondesc="";
				  ...
				  servicePstmt = connection.prepareStatement("select ACM.ACCOUNT_NO,ACM.FIRST_NAME,ACM.MIDDLE_NAME,ACM.LAST_NAME,ACM.ACC_BRANCH,to_char(ACM.DOB,'dd-mm-yyyy'),ACM.EMAILID,ACM.GENDER,ACM.SUPER_ADM,ACM.W_PRD_CODE,MCI.MOBILE_NUMBER,"
				  		+ "MCI.ADDRESS,MCI.ID_TYPE,MCI.ID_DETAILS,MCI.NATIONALITY,MCI.RL_LGA,MCI.R_STATE,MCI.COUNTRY,decode(ACM.STATUS,'B','Block','F','Inactive','A','Active','L','Deactivated','Active'),ACM.ID,NVL(ACM.USER_ID,' '),WAD.ACCT_NO,to_char(ACM.DATE_CREATED,'dd-mm-yyyy hh:mi:ss'),W_PRD_CODE,W_PRD_DESC,AGENCY_TYPE,ACM.ID,"
				  		+ "decode(ACM.USSD_STATUS,'A','Active','Deactivated'),decode(ACM.MOBILE_STATUS,'A','Active','Deactivated'),decode(ACM.POS_STATUS,'A','Active','Deactivated'),WAD.BALANCE,ACM.MARITAL_STATUS,ACM.MCC_NAME,ACM.BUSINESS_NAME from AGENT_CUSTOMER_MASTER ACM,AGENT_CONTACT_INFO MCI,WALLET_ACCT_DATA WAD where ACM.ID=MCI.CUST_ID and MCI.CUST_ID=WAD.CUST_ID and WAD.CUST_TYPE='AGENT' and ACM.CUSTOMER_CODE="+customercode+"");
				  serviceRS = servicePstmt.executeQuery();
				  JSONObject json = new JSONObject();
					 while(serviceRS.next())
						{
						...
						json.put("prodesc",serviceRS.getString(25));
						json.put("agenttype",serviceRS.getString(26));
						json.put("custid",serviceRS.getString(27));
						
						json.put("ussd_status",serviceRS.getString(28));
						json.put("mobile_status",serviceRS.getString(29));
						json.put("pos_status",serviceRS.getString(30));
						json.put("walletbalance",serviceRS.getString(31));
						
						json.put("martStatus",serviceRS.getString(32));
						json.put("mcc",serviceRS.getString(33));
						json.put("bsnname",serviceRS.getString(34));
	...
	/*public ResponseDTO AgentPinResetAck(RequestDTO requestDTO) {
		...
		try {
			...
			connection.setAutoCommit(false);
				...
				String msg="Dear "+userid+", Please find the your txn Pin "+resp.getString("pin");
				String str = "{'recipients': ['"+mnumber+"'],'sender': 'Xpress','message': '"+msg+"'}";
			
				ExternalServices.sendSMS(str,makerId);
	...
	public ResponseDTO AgentPinResetAck(RequestDTO requestDTO) {
		...
		try {
			...
			connection = DBConnector.getConnection();
				...
				insQRY = "{call WALLETACCOUNTSPKG.AGENTPINRESET(?,?,?,?,?)}";
	...
	public ResponseDTO AgentPasswordResetAck(RequestDTO requestDTO) {
		...
		try {
			...
			connection.setAutoCommit(false);
				...
				String password =  CommonServiceDao.getGenPass();
				String passhash =  CommonServiceDao.b64_sha256(password);
				
				pstmt = connection.prepareStatement("UPDATE  WALLET_AGENT_LOGIN set USER_PASSWORD=? where CUST_ID=?");
				...
				String msg="Dear "+userid+", Please find the your Password "+password;
				String str = "{'recipients': ['"+mnumber+"'],'sender': 'Xpress','message': '"+msg+"'}";
				...
				json.put("transCode", "pinreset");
				json.put("channel", "WEB");
				json.put("message", "Acknowledgment :: Password Reset for  USER ID "+userid);
				json.put("ip", remoteip);
				json.put("det1", "");
				json.put("det2", "");
				json.put("det3", "");
	...
	public ResponseDTO AgentBlockAck(RequestDTO requestDTO) {
		...
		try {
			...
			connection = DBConnector.getConnection();
				...
				insQRY = "{call WALLETACCOUNTSPKG.AGENTCHANNELSTATUS(?,?,?,?,?)}";
	...
	public ResponseDTO AgentStatusAck(RequestDTO requestDTO) {
		...
		try {
			...
			connection = DBConnector.getConnection();
				...
				insQRY = "{call WALLETACCOUNTSPKG.AGENTSTATUSACTIVE(?,?,?,?,?)}";
...
public ResponseDTO ApprovalAll(RequestDTO requestDTO) {
	...
	try {
		...
		queries ="select distinct AR.RELATION,AR.RELATION_CODE,AR.RELATION_ACTION,"
				+ "(SELECT COUNT(*) FROM AUTH_PENDING WHERE AUTH_FLAG=AR.RELATION_CODE AND STATUS='P' AND MAKER_BRCODE=UI.CLUSTER_ID),"
				+ "(SELECT COUNT(*) FROM AUTH_PENDING WHERE AUTH_FLAG=AR.RELATION_CODE AND STATUS='C' AND MAKER_BRCODE=UI.CLUSTER_ID),"
				+ "(SELECT COUNT(*) FROM AUTH_PENDING WHERE AUTH_FLAG=AR.RELATION_CODE AND STATUS='R' AND MAKER_BRCODE=UI.CLUSTER_ID),"
				+ "(SELECT COUNT(*) FROM AUTH_PENDING WHERE AUTH_FLAG=AR.RELATION_CODE AND STATUS='D' AND MAKER_BRCODE=UI.CLUSTER_ID) "
...
public ResponseDTO ApprovalDetails(RequestDTO requestDTO) {
	...
	try {
		...
		queries ="select  AM.HEADING_NAMES,AM.AUTH_CODE,AM.ACTION_URL,"
				+ "(SELECT COUNT(*) FROM AUTH_PENDING AP WHERE AP.AUTH_CODE=AM.AUTH_CODE AND AP.STATUS='P' AND MAKER_BRCODE=UI.CLUSTER_ID),"
				+ "(SELECT COUNT(*) FROM AUTH_PENDING AP WHERE AP.AUTH_CODE=AM.AUTH_CODE AND AP.STATUS='C' AND MAKER_BRCODE=UI.CLUSTER_ID),"
				+ "(SELECT COUNT(*) FROM AUTH_PENDING AP WHERE AP.AUTH_CODE=AM.AUTH_CODE AND AP.STATUS='R' AND MAKER_BRCODE=UI.CLUSTER_ID),"
				+ "(SELECT COUNT(*) FROM AUTH_PENDING AP WHERE AP.AUTH_CODE=AM.AUTH_CODE AND AP.STATUS='D' AND MAKER_BRCODE=UI.CLUSTER_ID) "
				+ "from USER_INFORMATION UI,USER_LOGIN_CREDENTIALS ULC,USER_LINKED_ACTION ULA,USER_ACTION_LINKS UAL,AUTH_MASTER AM,AUTH_RELATION AR "
		+"where UI.COMMON_ID=ULC.COMMON_ID AND "
		+"ULA.GROUP_ID=UI.USER_GROUPS  AND  "
		+"ULA.ID=UAL.ID  AND "
...
if(application.equalsIgnoreCase("MOBILE_NO")){
				...
				/*qrey="select * from ((select EXT_TXN_REF_NO,USER_ID,DR_ACCOUNT,CR_ACCOUNT,TO_CHAR(NVL(AMOUNT,0)),CHANNEL,'Success',to_char(TXN_STAMP,'dd-mm-yyyy hh:mi:ss'),TXN_STAMP as tdate,DECODE(SERVICECODE,'AGCASHDEP','Cash Deposite','cash withdrawal') as scode,TO_CHAR(NVL(TXN_AMT,0)),TO_CHAR(NVL(FEE_AMT,0))  " + 
						"from WALLET_FIN_TXN where USER_ID='"+fname+"' and SERVICECODE in ('AGCASHDEP','AGCASHWTHD') AND substr(TXN_REF_NO,1,1)!='R' ) " + 
						"union all  " + 
						"(select AFT.PAY_REF_NO,WFT.USER_ID,WFT.DR_ACCOUNT,AFT.BEN_PAYBILL_ACTNO,TO_CHAR(NVL(WFT.AMOUNT,0)),WFT.CHANNEL,decode(AFT.RESP_CODE,'00','Success','Failed'),to_char(WFT.TXN_STAMP,'dd-mm-yyyy hh:mi:ss'),WFT.TXN_STAMP  as tdate,(SELECT SERVICEDESC from BANK_SERVICE_CODE_MASTER where SERVICECODE=TRANS_TYPE),TO_CHAR(NVL(WFT.TXN_AMT,0)),TO_CHAR(NVL(WFT.FEE_AMT,0))   " + 
						"from WALLET_AGNCORE_TXNTBL AFT LEFT JOIN WALLET_FIN_TXN WFT ON AFT.PAY_REF_NO=WFT.ext_txn_ref_no where USER_ID='"+fname+"' AND substr(TXN_REF_NO,1,1)!='R' ) " + 
						"union all  " + 
						"(select AFT.PAY_REF_NO,AFT.USERID,AFT.DR_ACCT_NO,AFT.BEN_PAYBILL_ACTNO,TO_CHAR(NVL(AFT.TRNS_AMT,0)),AFT.CHANNEL,decode(AFT.RESP_CODE,'00','Success','Failed'),to_char(WFT.TXN_STAMP,'dd-mm-yyyy hh:mi:ss'),WFT.TXN_STAMP  as tdate,(SELECT SERVICEDESC from BANK_SERVICE_CODE_MASTER where SERVICECODE=TRANS_TYPE),TO_CHAR(NVL(WFT.TXN_AMT,0)),TO_CHAR(NVL(WFT.FEE_AMT,0))   " + 
						"from WALLET_AGNCORE_TXNTBL AFT LEFT JOIN WALLET_FIN_TXN WFT ON AFT.PAY_REF_NO=WFT.ext_txn_ref_no where USER_ID='"+fname+"' AND trans_type='AGENTFUND' ) " + 
						"union all  " + 
						...
						"from TBL_TRANLOG_ALL TTA LEFT JOIN WALLET_FIN_TXN WFT ON TTA.BANKRRN=WFT.ext_txn_ref_no where TTA.APPROVEDBY='"+fname+"' and TTA.INTERNALID=WFT.txn_ref_no AND substr(TXN_REF_NO,1,1)!='R' )) order by tdate desc";
				*/
...
public ResponseDTO agentrequestdetails(RequestDTO requestDTO) {
	...
	try {
		...
		if(application.equalsIgnoreCase("PAYMENT_REF")) {
			...
			qrey="select WFTP.EXT_TXN_REF_NO,WFTP.USER_ID as userid,WFTP.DR_ACCOUNT,WFTP.CR_ACCOUNT,NVL(WFTP.AMOUNT,0),WFTP.CHANNEL as cchannel,decode(substr(WFTP.txn_ref_no,1,1),'R','Failed','Success'),to_char(WFTP.TXN_STAMP,'dd-mm-yyyy hh:mi:ss'),WFTP.TXN_STAMP  as tdate,(SELECT BSCM.SERVICEDESC from BANK_SERVICE_CODE_MASTER BSCM where BSCM.SERVICECODE=WFTP.SERVICECODE),ltrim(NVL(WFTP.TXN_AMT,0),0),NVL(WFTP.FEE_AMT,0) from wallet_fin_txn WFTP"
					+ " where not exists (select 1 from wallet_fin_txn ft where ft.txn_ref_no='R'||WFTP.txn_ref_no) and substr(WFTP.txn_ref_no,1,1)<>'R' AND WFTP.EXT_TXN_REF_NO='"+mnumber+"' order by tdate desc";
			
		}
		
		if(application.equalsIgnoreCase("WALLET_REF")) {
			
			qrey="select WFTP.EXT_TXN_REF_NO,WFTP.USER_ID as userid,WFTP.DR_ACCOUNT,WFTP.CR_ACCOUNT,NVL(WFTP.AMOUNT,0),WFTP.CHANNEL as cchannel,decode(substr(WFTP.txn_ref_no,1,1),'R','Failed','Success'),to_char(WFTP.TXN_STAMP,'dd-mm-yyyy hh:mi:ss'),WFTP.TXN_STAMP  as tdate,(SELECT BSCM.SERVICEDESC from BANK_SERVICE_CODE_MASTER BSCM where BSCM.SERVICECODE=WFTP.SERVICECODE),ltrim(NVL(WFTP.TXN_AMT,0),0),NVL(WFTP.FEE_AMT,0) from wallet_fin_txn WFTP"
					+ " where not exists (select 1 from wallet_fin_txn ft where ft.txn_ref_no='R'||WFTP.txn_ref_no) and substr(WFTP.txn_ref_no,1,1)<>'R' AND WFTP.TXN_REF_NO='"+mnumber+"' order by tdate desc";
...
public ResponseDTO agentrequestinfo(RequestDTO requestDTO) {
	...
	try {
		...
		/*if(requesttype.equalsIgnoreCase("Fund Transfer Other Bank")) {
		...
		qrey="select WFTP.EXT_TXN_REF_NO,WFTP.USER_ID as userid,WFTP.DR_ACCOUNT,WFTP.txn_ref_no,NVL(WFTP.AMOUNT,0),WFTP.CHANNEL as cchannel,decode(substr(WFTP.txn_ref_no,1,1),'R','Failed','Success'),to_char(WFTP.TXN_STAMP,'dd-mm-yyyy hh:mi:ss'),(SELECT BSCM.SERVICEDESC from BANK_SERVICE_CODE_MASTER BSCM where BSCM.SERVICECODE=WFTP.SERVICECODE),ltrim(NVL(WFTP.TXN_AMT,0),0),NVL(WFTP.FEE_AMT,0),WFTP.SERVICECODE from wallet_fin_txn WFTP"
				+ " where not exists (select 1 from wallet_fin_txn ft where ft.txn_ref_no='R'||WFTP.txn_ref_no) and substr(WFTP.txn_ref_no,1,1)<>'R' AND WFTP.EXT_TXN_REF_NO='"+refno+"' ";
			...
			while (getlmtfeeRs.next()) {
				...
				json.put("TRNS_AMT", getlmtfeeRs.getString(5));
				json.put("CHANNEL", getlmtfeeRs.getString(6));
				json.put("WAL_RESP", getlmtfeeRs.getString(7));
				json.put("TRANS_DATE", getlmtfeeRs.getString(8));
				json.put("SERVICECODE", getlmtfeeRs.getString(9));
				json.put("TXN_AMT", getlmtfeeRs.getString(10));
				json.put("FEE_AMT", getlmtfeeRs.getString(11));
				json.put("TXN_CODE", getlmtfeeRs.getString(12));
...
public ResponseDTO agentrequestinfoAck(RequestDTO requestDTO) {
	...
	try {
		...
		pstmt1 = connection.prepareStatement("SELECT count(*) FROM WALLET_SUCCREV_REQUEST WHERE PAYMENTREFERENCE=?");
		pstmt1.setString(1, paymentrefno);
		...
		while (rs1.next()) {
			...
			if(rs1.getInt(1)==0) {
				connection.setAutoCommit(false);
				pstmt = connection.prepareStatement("INSERT INTO WALLET_SUCCREV_REQUEST(PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,TXN_TYPE,REQUEST_TYPE,REMARKS,MAKER_ID,MAKER_DT,STATUS,ACCT_NO,BATCHID,DEBITNARRATION,TRANS_DATE,ACTION_TYPE,P_SEQNO) VALUES(?,?,?,?,?,?,?,?,SYSDATE,'P',?,?,?,?,?,WALLET_SUCCREV_REQUEST_SEQ.nextval)");
...
public ResponseDTO agentrequestApproval(RequestDTO requestDTO) {
	...
	try {
		...
		sb.append("(select count(*) from WALLET_SUCCREV_REQUEST where STATUS='C' AND TXN_TYPE='WALAGNOTBANK') FROM DUAL");
		...
		sb.append("SELECT (select count(*) from WALLET_SUCCREV_REQUEST where STATUS='P' ),");
		sb.append("(select count(*) from WALLET_SUCCREV_REQUEST where STATUS='C' ) FROM DUAL");
		...
		while (getlmtfeeRs.next()) {
			...
			json = new JSONObject();
			json.put("COUNT_PENDING", getlmtfeeRs.getString(1));
			json.put("COUNT_COMPLETED", getlmtfeeRs.getString(2));
...
public ResponseDTO unsettlrequestapproval(RequestDTO requestDTO) {
	...
	try {
		...
		sb.append("SELECT (select count(*) from WALLET_UNSETTLE_DATA where STATUS='P' ),");
		sb.append("(select count(*) from WALLET_UNSETTLE_DATA where STATUS='C' ) FROM DUAL");
...
public ResponseDTO unsettlrequestapprovaldetails(RequestDTO requestDTO) {
	...
	try {
		...
		if(application.equalsIgnoreCase("PENDING")){
			...
			qrey="select PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,txn_TYPE,TRANS_DATE,MAKER_ID,MAKER_DT,ACCT_NO,REQUEST_TYPE from WALLET_UNSETTLE_DATA  where  STATUS='P' ";   

		}else if(application.equalsIgnoreCase("COMPLETED")){
			qrey="select PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,txn_TYPE,TRANS_DATE,MAKER_ID,MAKER_DT,ACCT_NO,REQUEST_TYPE from WALLET_UNSETTLE_DATA  where  STATUS='C' ";   
...
public ResponseDTO agentrequestapprovaldetails(RequestDTO requestDTO) {
	...
	try {
		...
		if(application.equalsIgnoreCase("PENDING")){
			...
			qrey="select PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,REQUEST_TYPE,TRANS_DATE,MAKER_ID,MAKER_DT,ACCT_NO,ACTION_TYPE,P_SEQNO from WALLET_SUCCREV_REQUEST  where  STATUS='P' ";   

		}else if(application.equalsIgnoreCase("COMPLETED")){
			qrey="select PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,REQUEST_TYPE,TRANS_DATE,MAKER_ID,MAKER_DT,ACCT_NO,ACTION_TYPE,P_SEQNO from WALLET_SUCCREV_REQUEST  where  STATUS='C' ";   
...
public ResponseDTO agentrequestapprovalinfo(RequestDTO requestDTO) {
	...
	try {
		...
		if(application.equalsIgnoreCase("PENDING")){
			qrey="select PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,TXN_TYPE,TRANS_DATE,MAKER_ID,MAKER_DT,REMARKS,ACTION_TYPE,REQUEST_TYPE,BATCHID,ACCT_NO,NVL(DEBITNARRATION,' '),P_SEQNO from WALLET_SUCCREV_REQUEST  where STATUS='P'  AND P_SEQNO='"+refno+"'";   

		}else if(application.equalsIgnoreCase("COMPLETED")){
			qrey="select PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,TXN_TYPE,TRANS_DATE,MAKER_ID,MAKER_DT,REMARKS,ACTION_TYPE,REQUEST_TYPE,BATCHID,ACCT_NO,NVL(DEBITNARRATION,' '),P_SEQNO from WALLET_SUCCREV_REQUEST  where STATUS='C'  AND P_SEQNO='"+refno+"'";   
...
public ResponseDTO unsettlrequestapprovalinfo(RequestDTO requestDTO) {
	...
	try {
		...
		if(application.equalsIgnoreCase("PENDING")){
			qrey="select PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,txn_TYPE,TRANS_DATE,MAKER_ID,MAKER_DT,REMARKS,txn_TYPE,REQUEST_TYPE,BATCHID,ACCT_NO,NVL(DEBITNARRATION,' ') from WALLET_UNSETTLE_DATA where STATUS='P'  AND PAYMENTREFERENCE='"+refno+"'";   

		}else if(application.equalsIgnoreCase("COMPLETED")){
			qrey="select PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,txn_TYPE,TRANS_DATE,MAKER_ID,MAKER_DT,REMARKS,txn_TYPE,REQUEST_TYPE,BATCHID,ACCT_NO,NVL(DEBITNARRATION,' ') from WALLET_UNSETTLE_DATA  where STATUS='C'  AND PAYMENTREFERENCE='"+refno+"'";   
			...
			while (getlmtfeeRs.next()) {
				...
				json.put("ACTION_TYPE", getlmtfeeRs.getString(11));
				json.put("REQUEST_TYPE", getlmtfeeRs.getString(10));
				json.put("WALLETNO", getlmtfeeRs.getString(12));
				json.put("BATCHID", getlmtfeeRs.getString(12));
				json.put("ACCT_NO", getlmtfeeRs.getString(13));
				json.put("DEBITNARRATION", getlmtfeeRs.getString(14));
				//json.put("DETAILS", ((getlmtfeeRs.getString(14).replaceAll(":", "<font color=\"red\">"))).replaceAll("#", "</font><br>"));
				json.put("DETAILS","");
				//json.put("P_SEQNO", getlmtfeeRs.getString(15));
			}
...
public ResponseDTO agentrequestinfoApprovalAck(RequestDTO requestDTO) {
	...
	try {
		...
		String waccountno=requestJSON.getString("waccountno");
		//String application=(requestJSON.getString("application")).replace("_PENDING", "");
		String actiontype=requestJSON.getString("actiontype");
		String application=requestJSON.getString("application");
		String requesttype=requestJSON.getString("requesttype");
		...
		if(actiontype.equalsIgnoreCase("Wallet Reversal")) {
			...
			logger.debug("[Success Reversal wallet ref no : "+batchid+"]");
			...
			logger.debug("[Success Reversal output Message : "+cstmt.getString(5)+"]");
			logger.debug("[Success Reversal output : "+cstmt.getString(4)+"]");
			...
			if ((cstmt.getString(5)).contains("1")) {
				...
				responseDTO.addError("Reveral Failed : Response :"+cstmt.getString(4));
			}
			...
			connection.setAutoCommit(false);
			pstmt = connection.prepareStatement("UPDATE WALLET_SUCCREV_REQUEST SET STATUS='R',CHECKER_ID=?,CHECKER_DATE=sysdate,RESPENSE_MESSAGE=? WHERE PAYMENTREFERENCE=? AND P_SEQNO=?");
			pstmt.setString(1, makerid);
			pstmt.setString(2, "Rejected Successfully  Reason for "+reason);
			pstmt.setString(3, paymentrefno);
			pstmt.setString(4, refno);
			pstmt.executeUpdate();
			
			pstmt = connection.prepareStatement("insert into  WALLET_SUCCREV_REQUEST_HIST select * from  WALLET_SUCCREV_REQUEST WHERE PAYMENTREFERENCE=? AND P_SEQNO=?");
			pstmt.setString(1, paymentrefno);
			pstmt.setString(2, refno);
			pstmt.executeUpdate();
			
			pstmt = connection.prepareStatement("delete from WALLET_SUCCREV_REQUEST WHERE PAYMENTREFERENCE=? AND P_SEQNO=?");
			pstmt.setString(1, paymentrefno);
			pstmt.setString(2, refno);
			pstmt.executeUpdate();
			
			connection.commit();
			responseDTO.addError("Rejected Successfully ..");
		}
		
		/*if(txntype.equalsIgnoreCase("Fund Transfer Other Bank")) {
			corerequest="NIP_CORE_SUCCESS_TRANS_REV";
			walletrequest="NIP_WALLET_SUCCESS_TRANS_REV";
		}
		
		if(txntype.equalsIgnoreCase("Pay Bill")) {
			corerequest="PAYBILL_CORE_SUCCESS_TRANS_REV";
			walletrequest="PAYBILL_WALLET_SUCCESS_TRANS_REV";
		}
		
		if(txntype.equalsIgnoreCase("Recharge")) {
			...
			/*if(actiontype.equalsIgnoreCase("Wallet Reversal")) {
				JSONObject json1 = JSONObject.fromObject(SettlementServiceCall.walletReversal(walletrequest, makerid, batchid,application));
...
public ResponseDTO unsettlrequestinfoapprovalack(RequestDTO requestDTO) {
	
	responseDTO=new ResponseDTO();
	requestJSON=new JSONObject();
	responseJSON=new JSONObject();
	
	logger.debug("inside [AgentDAO][unsettlrequestinfoapprovalack]");
	...
	try {
		...
		System.out.println("reeqtype"+actiontype);
		String refno=requestJSON.getString("refno");
		
		//Date date1=new SimpleDateFormat("dd/MM/yyyy").parse(txndatetime);
		
		//System.out.println("date1"+date1);
		
		String makerid=requestJSON.getString(CevaCommonConstants.MAKER_ID);
		
		System.out.println("after dtaa vali");
		
		if(requesttype.equalsIgnoreCase("Approval")) {
			
		
		
		if(actiontype.equalsIgnoreCase("DATAINSERTION")) {
			...
			//pstmt = connection.prepareStatement(" insert into wallet_unsettelment_tbl (UNSETID,USERID,TXNREF,EXTTXNREF,SERVICECODE,TXNAMOUNT,TXNDATE,SETDRCR,SETAMOUNT,CHANNEL,STATUS,REVERSAL_REF_NO,NARRATION,AMOUNT,FEE) values (WAL_UNSET_SEQ.NEXTVAL,userid,batchid,paymentrefno,txntype,txnamt,'','C',txnamt,channel,'B',batchid,Narration,txnamt,'0')");

			//pstmt = connection.prepareStatement(" insert into wallet_unsettelment_tbl (UNSETID,USERID,TXNREF,EXTTXNREF,SERVICECODE,TXNAMOUNT,TXNDATE,SETDRCR,SETAMOUNT,CHANNEL,STATUS,REVERSAL_REF_NO,NARRATION,AMOUNT,FEE) values (WAL_UNSET_SEQ.NEXTVAL,?,?,?,?,?,?,'C',?,?,'B',?,?,?,'0')");
			
			pstmt = connection.prepareStatement("insert into wallet_unsettelment_tbl (UNSETID,USERID,TXNREF,EXTTXNREF,SERVICECODE,TXNAMOUNT,TXNDATE,SETDRCR,SETAMOUNT,CHANNEL,STATUS,REVERSAL_REF_NO,NARRATION,AMOUNT,FEE) (SELECT  WAL_UNSET_SEQ.NEXTVAL,wst.USERID,wst.WALLET_REF_NO,wst.pay_ref_no,wst.TRANS_TYPE,wst.TRNS_AMT,TRANS_DATE,'C',wst.TRNS_AMT,wst.CHANNEL,'B',wst.WALLET_REF_NO,wst.DR_NARRATION,wst.TRNS_AMT,'0'from WALLET_AGNCORE_TXNTBL wst where wst.USERID =?   and  wst.TRNS_AMT=? and  RESP_CODE!='00'  and pay_ref_no=? and pay_ref_no not in ( select EXTTXNREF from WALLET_unSETTELMENT_TBL) and pay_reF_no not in( select EXTTXNREF from WALLET_SETTELMENT_TBL) and pay_reF_no not in( select PAYMENTREFERENCE from wallet_unsettle_data where status in ('C')))"); 
			System.out.println("after query");
			pstmt.setString(1,userid);
			pstmt.setString(2, txnamt);
			pstmt.setString(3,paymentrefno);
			
			//pstmt = connection.prepareStatement(insQR);
			int i = pstmt.executeUpdate();
			if (i >0) {
				System.out.println("kailash here :: ");
				connection.setAutoCommit(false);
				pstmt = connection.prepareStatement("UPDATE WALLET_UNSETTLE_DATA SET STATUS='C',CHECKER_ID=?,CHECKER_DATE=sysdate,SERVICE_RESP=? WHERE PAYMENTREFERENCE=?");
				pstmt.setString(1, makerid);
				pstmt.setString(2, "Wallet Response :SUCCESS");
				pstmt.setString(3, paymentrefno);
				//pstmt.setString(4, refno);
				...
				connection.setAutoCommit(false);
				pstmt = connection.prepareStatement("UPDATE WALLET_UNSETTLE_DATA SET STATUS='F',CHECKER_ID=?,CHECKER_DATE=sysdate,SERVICE_RESP=? WHERE PAYMENTREFERENCE=?");
				pstmt.setString(1, makerid);
				pstmt.setString(2, "Wallet Response :FAIL");
				pstmt.setString(3, paymentrefno);
				//pstmt.setString(4, refno);
				pstmt.executeUpdate();
				connection.commit();
			...
			connection.setAutoCommit(false);
			pstmt = connection.prepareStatement("UPDATE WALLET_UNSETTLE_DATA SET STATUS='R',CHECKER_ID=?,CHECKER_DATE=sysdate,SERVICE_RESP=? WHERE PAYMENTREFERENCE=? ");
			pstmt.setString(1, makerid);
			pstmt.setString(2, "Rejected Successfully  Reason for "+reason);
			pstmt.setString(3, paymentrefno);
			//pstmt.setString(4, refno);
			...
			pstmt = connection.prepareStatement("insert into  WALLET_UNSETTLE_DATA_HIST select * from  WALLET_UNSETTLE_DATA WHERE PAYMENTREFERENCE=?");
			pstmt.setString(1, paymentrefno);
			//pstmt.setString(2, refno);
			pstmt.executeUpdate();
			
			pstmt = connection.prepareStatement("delete from WALLET_UNSETTLE_DATA WHERE PAYMENTREFERENCE=?");
			pstmt.setString(1, paymentrefno);
			//pstmt.setString(2, refno);
			pstmt.executeUpdate();
...
public ResponseDTO walletaccinfo(RequestDTO requestDTO) {
	...
	try {
		...
		String remoteip=requestJSON.getString("remoteip");
				...
				qrey = "select  wad.acct_no,wad.ACCT_NAME,wad.CUST_TYPE,wad.balance,MOBILE_NUMBER from wallet_acct_data wad,AGENT_CONTACT_INFO aci  where aci.CUST_ID=wad.CUST_ID AND balance>0  order by acct_no" ;	
			...
			qrey = "select  sum(balance) from wallet_acct_data where balance>0 " ;	
...
public ResponseDTO walletbalancesheet(RequestDTO requestDTO) {
	...
	try {
		...
		String fromdate=requestJSON.getString("fromdate");
			...
			qrey = "select sum(decode(substr(TXN_REF_NO,1,1),'R',-FEE_AMT,FEE_AMT)) from WALLET_FIN_TXN where trunc(TXN_STAMP)<to_date('"+fromdate+"','dd/mm/yyyy')" ;	
			...
			while (getlmtfeeRs.next()) {
				...
				resultJson.put("FEE_AMT", getlmtfeeRs.getString(1));
			}
...
public ResponseDTO viewReconcileinfo(RequestDTO requestDTO) {
	...
	System.out.println("in viewReconcileinfo");
...
public ResponseDTO unsettlemetdatewiseinfo(RequestDTO requestDTO) {

	logger.debug(" Inside unsettlemetdatewiseinfo data .. ");
	...
	*/
		...
		/*if((requestJSON.getString("servicecode")).equalsIgnoreCase("ALL")) {
		 qrey1 = "select wst.USERID,wst.TXNREF,wst.EXTTXNREF,wst.SERVICECODE,wst.TXNAMOUNT,wst.TXNDATE,wst.CHANNEL,DECODE(wst.SETDRCR,'C','Credit','Debit') from WALLET_SETTELMENT_TBL wst,WALLET_UNSETTELMENT_TBL wust where wst.EXTTXNREF =wust.EXTTXNREF and TRUNC(wst.TXNDATE) between to_date('"+fromdate+"','dd/mm/yyyy') and to_date('"+todate+"','dd/mm/yyyy') order by TXNDATE desc";
			...
			*/
			...
			logger.debug("before constructing query");
			logger.debug("txndate==="+requestJSON.getString("trnsdate"));
			 qrey1 = "select wst.USERID,wst.WALLET_REF_NO,wst.pay_ref_no,wst.TRANS_TYPE,wst.TRNS_AMT,wst.TRANS_DATE,wst.CHANNEL,RESP_CODE from WALLET_AGNCORE_TXNTBL wst where wst.USERID ='"+requestJSON.getString("userid")+"'  and TRUNC(wst.TRANS_DATE) =to_date('"+requestJSON.getString("trnsdate")+"','dd-mm-yyyy') and  wst.TRNS_AMT='"+requestJSON.getString("transamt")+"' and  RESP_CODE!='00'   and pay_ref_no not in ( select EXTTXNREF from WALLET_unSETTELMENT_TBL) and pay_reF_no not in ( select EXTTXNREF from WALLET_SETTELMENT_TBL)  and pay_reF_no not in ( select PAYMENTREFERENCE from WALLET_UNSETTLE_DATA ) ";
			
			logger.debug("after  constructing query"+qrey1);
		getlmtfeePstmt = connection.prepareStatement(qrey1);
			...
			while (getlmtfeeRs.next()) {
				...
				json.put("TXNDATE", getlmtfeeRs.getString(6));
				json.put("CHANNEL", getlmtfeeRs.getString(7));
				json.put("RESPCODE", getlmtfeeRs.getString(8));
		...
		e.printStackTrace();
		logger.debug("Got Exception in getting tramasction details Details ["
				+ e.getMessage() + "]");
	} finally {
...
public ResponseDTO unsettlemetreportinfo(RequestDTO requestDTO) {
	...
	*/
		...
		if((requestJSON.getString("servicecode")).equalsIgnoreCase("ALL")) {
			...
			qrey = " select USERID,TXNREF,EXTTXNREF,SERVICECODE,TXNAMOUNT,TXNDATE,CHANNEL,DECODE(SETDRCR,'C','Credit','Debit')  from WALLET_UNSETTELMENT_TBL t1 where  not exists  ( select 1 from WALLET_SETTELMENT_TBL t2 where t2.EXTTXNREF=t1.EXTTXNREF)  and not exists ( select 1 from WALLET_UNSETTLE_REQUEST t3 where t3.PAYMENTREFERENCE=t1.EXTTXNREF) and not exists ( select 1 from FILE_UPLOAD_UNSETTLE t4 where t4.TXN_REFERNCE_NO=t1.EXTTXNREF)  order by TXNDATE desc";
			...
			qrey = " select USERID,TXNREF,EXTTXNREF,SERVICECODE,TXNAMOUNT,TXNDATE,CHANNEL,DECODE(SETDRCR,'C','Credit','Debit')  from WALLET_UNSETTELMENT_TBL t1 where  not exists  ( select 1 from WALLET_SETTELMENT_TBL t2 where t2.EXTTXNREF=t1.EXTTXNREF)  and not exists ( select 1 from WALLET_UNSETTLE_REQUEST t3 where t3.PAYMENTREFERENCE=t1.EXTTXNREF) and not exists ( select 1 from FILE_UPLOAD_UNSETTLE t4 where t4.TXN_REFERNCE_NO=t1.EXTTXNREF) and SERVICECODE='"+requestJSON.getString("servicecode")+"' order by TXNDATE desc";
		...
		getlmtfeePstmt = connection.prepareStatement(qrey);
			...
			while (getlmtfeeRs.next()) {
				...
				resultJson.put("TOT_BALANCE", getlmtfeeRs.getString(1));
				resultJson.put("COUNT", getlmtfeeRs.getString(2));
				resultJson.put("SETTLEMET", "Unsettlement"+getlmtfeeRs.getString(3));
			}
			...
			settstr="select wst.userid,wst.txnref,wst.exttxnref,wst.servicecode,wst.txnamount,'' from wallet_unsettelment_tbl wst where exttxnref not in ( select exttxnref from wallet_settelment_tbl) and EXTTXNREF not in ( select PAYMENTREFERENCE from WALLET_UNSETTLE_REQUEST) and EXTTXNREF not in ( select TXN_REFERNCE_NO from FILE_UPLOAD_UNSETTLE) order by txndate desc";
				...
				// settstr="select
				// '\"'||wst.userid||'\"','\"'||wst.txnref||'\"','\"'||wst.exttxnref||'\"','\"'||wst.servicecode||'\"','\"'||wst.txnamount||'\"','\"'||CHANNEL||'\"',''
			...
			settstr="select '\"'||wst.userid||'\"','\"'||wst.txnref||'\"','\"'||wst.exttxnref||'\"','\"'||wst.servicecode||'\"','\"'||wst.txnamount||'\"','\"'||wst.TXNDATE||'\"','\"'||CHANNEL||'\"','' from wallet_unsettelment_tbl wst where  not exists  ( select 1 from wallet_settelment_tbl t2 where t2.EXTTXNREF=wst.EXTTXNREF) and  not exists ( select 1 from WALLET_UNSETTLE_REQUEST  t3 where t3.PAYMENTREFERENCE=wst.EXTTXNREF) and  not exists ( select 1 from FILE_UPLOAD_UNSETTLE t4 where t4.TXN_REFERNCE_NO=wst.EXTTXNREF) and SERVICECODE='"+requestJSON.getString("servicecode")+"' order by txndate desc";
					 
			}
			String heading="USER_ID,TXN_REFERNCE_NO,EXTREFERNCE_NO,SERVICE_CODE,TXN_AMOUNT,TXNDATE,CHANNEL,ACTION";
			logger.debug(" heading  the query"+heading);
...
public ResponseDTO unsettlementdetailsrequestinfo(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("unsettlementdetailsrequestinfo ");
		...
		System.out.println("refno value is "+refno);
		System.out.println("requesttype value is"+requesttype);
		
		
		
		
		//qrey = "select USERID,TXNREF,EXTTXNREF,SERVICECODE,TXNAMOUNT,TXNDATE,CHANNEL,DECODE(SETDRCR,'C','Credit','Debit'),NARRATION  from WALLET_UNSETTELMENT_TBL where EXTTXNREF not in ( select EXTTXNREF from WALLET_SETTELMENT_TBL) and TXNREF='"+refno+"' and SERVICECODE='"+requesttype+"'";
		qrey ="select wst.USERID,wst.WALLET_REF_NO,wst.pay_ref_no,wst.TRANS_TYPE,wst.TRNS_AMT,wst.TRANS_DATE,wst.CHANNEL,RESP_CODE,DR_NARRATION from WALLET_AGNCORE_TXNTBL wst where pay_ref_no not in ( select EXTTXNREF from WALLET_unSETTELMENT_TBL) and pay_reF_no not in ( select EXTTXNREF from WALLET_SETTELMENT_TBL) and WALLET_REF_NO='"+refno+"' and trans_type='"+requesttype+"' and  RESP_CODE!='00' order by TRANS_DATE desc";
...
public ResponseDTO unsettlementrequestinfoAck(RequestDTO requestDTO) {
	...
	try {
		...
		String remarks=requestJSON.getString("remarks");
		//String requesttype=requestJSON.getString("requesttype");
		String requesttype="DATAINSERTION";
		//String waccountno=requestJSON.getString("waccountno");
		String makerid=requestJSON.getString(CevaCommonConstants.MAKER_ID);
		
		
		connection.setAutoCommit(false);
		pstmt = connection.prepareStatement("INSERT INTO WALLET_UNSETTLE_DATA(PAYMENTREFERENCE,TRNS_AMT,CHANNEL,USER_ID,TXN_TYPE,REQUEST_TYPE,REMARKS,MAKER_ID,MAKER_DT,STATUS,ACCT_NO,BATCHID,DEBITNARRATION,TRANS_DATE) VALUES(?,?,?,?,?,?,?,?,SYSDATE,'P',?,?,?,?)");
		pstmt.setString(1, paymentrefno);
		pstmt.setString(2, txnamt);
		pstmt.setString(3, channel);
		pstmt.setString(4, userid);
...
public ResponseDTO settlementrequestinfoApprovalAck(RequestDTO requestDTO) {
	...
	try {
		...
		String walleType = requestJSON.getString("waccountno");
		
		connection.setAutoCommit(false);
		if((requestJSON.getString("requesttype")).equalsIgnoreCase("Approval")) {
			
			
			pstmt = connection.prepareStatement("UPDATE WALLET_UNSETTLE_REQUEST SET STATUS='I',CHECKER_ID=?,CHECKER_DATE=sysdate WHERE PAYMENTREFERENCE=?");
			pstmt.setString(1, makerid);
			pstmt.setString(2, paymentrefno);
			pstmt.executeUpdate();
			connection.commit();
			
			if (walleType.equalsIgnoreCase("Discard") ) 
			{
				
				logger.info("Wallet Discard Calling......");
				
				pstmt = connection.prepareStatement(("INSERT INTO WALLET_SETTELMENT_TBL (SETID,USERID,TXNREF,EXTTXNREF,SERVICECODE,TXNAMOUNT,TXNDATE,SETDRCR,SETAMOUNT,CHANNEL,STATUS,REVERSAL_REF_NO,NARRATION,AMOUNT,FEE) " + 
						"select WAL_SET_SEQ.NEXTVAL as val , USERID, TXNREF, EXTTXNREF, SERVICECODE, TXNAMOUNT, sysdate, SETDRCR, SETAMOUNT, CHANNEL, STATUS,'', NARRATION, AMOUNT,FEE " + 
						"from WALLET_UNSETTELMENT_TBL unt where EXTTXNREF=?"));
				
				pstmt.setString(1, paymentrefno);
				pstmt.executeUpdate();
				
				pstmt = connection.prepareStatement("UPDATE WALLET_UNSETTLE_REQUEST SET STATUS='C',CHECKER_ID=?,CHECKER_DATE=sysdate,SERVICE_RESP=? WHERE PAYMENTREFERENCE=?");
				pstmt.setString(1, makerid);
				pstmt.setString(2, paymentrefno);
				pstmt.setString(3, "00"+"--"+ "SUCCESS" );
				pstmt.executeUpdate();
			...
			logger.info("Wallet Reversal && agent funding Calling......");
			...
			if((json1.getString("respcode")).equalsIgnoreCase("00")){
				...
				pstmt.setString(1, makerid);
				pstmt.setString(2, paymentrefno);
				pstmt.setString(3, json1.getString("respcode")+"--"+json1.getString("respdesc"));
				pstmt.executeUpdate();
				
				
				responseDTO.addMessages("Success");
			}else{
				pstmt = connection.prepareStatement("UPDATE WALLET_UNSETTLE_REQUEST SET STATUS='F',CHECKER_ID=?,CHECKER_DATE=sysdate,SERVICE_RESP=? WHERE PAYMENTREFERENCE=?");
				pstmt.setString(1, makerid);
				pstmt.setString(2, paymentrefno);
				pstmt.setString(3, json1.getString("respcode")+"--"+json1.getString("respdesc"));
				pstmt.executeUpdate();
				...
				responseDTO.addError(json1.getString("respdesc"));
			}
			  ...
			  JSONObject json = new JSONObject();
				 while(serviceRS.next())
					...
					serviceRS.close();
					 ...
					 userReport ="select BALANCE from WALLET_COM_ACCT_DATA where ACCT_NO='0"+json.getString("walletaccountno")+"'";
		            	 ...
		            	 servicePstmt = connection.prepareStatement(userReport);
							serviceRS = servicePstmt.executeQuery();
							 ...
							 while(serviceRS.next())
								{
								 json.put("commissionunst",serviceRS.getString(1));
					...
					userReport ="select BEN_ACCT,BEN_BANK_CODE,BEN_BANK_NAME from AGENT_CASHOUT_ACT_TBL where USER_ID='"+json.getString("telephone")+"'";

			        servicePstmt = connection.prepareStatement(userReport);
					serviceRS = servicePstmt.executeQuery();
					json.put("cbankacc","");
					json.put("cbankacccode","");
					json.put("cbankname","");	
								
						while(serviceRS.next())
						{
							json.put("cbankacc",serviceRS.getString(1));
							json.put("cbankacccode",serviceRS.getString(2));
							json.put("cbankname",(serviceRS.getString(3)).replace("+", " "));
					 ...
					 userReport ="select rownum SNO,SM.STORE_NAME,SM.STORE_ID,SM.ADDRESS,SM.LOC_GOV,SM.STATE,DECODE(SM.STATUS,'B','BLOCK','F','INACTIVE','A','ACTIVE','L','DEACTIVATION') from AGENT_CUSTOMER_MASTER ACM,AGENT_CONTACT_INFO MCI,WALLET_ACCT_DATA WAD,STORE_MASTER SM "	
					 + "where ACM.ID=MCI.CUST_ID and MCI.CUST_ID=WAD.CUST_ID  and MCI.CUST_ID=SM.CUST_ID and WAD.CUST_TYPE='AGENT' AND "+ " ACM.CUSTOMER_CODE="+customercode+" ";
            	 ...
            	 servicePstmt = connection.prepareStatement(userReport);
					...
					JSONObject json1 = new JSONObject();
					 ...
					 while(serviceRS.next())
						{
						 json1=new JSONObject();
						 json1.put("SNO",serviceRS.getString(1));
						 json1.put("STORE_NAME",serviceRS.getString(2));
						 json1.put("STORE_ID",serviceRS.getString(3));
						 json1.put("ADDRESS",serviceRS.getString(4));
						 json1.put("LOC_GOV",serviceRS.getString(5));
						 json1.put("STATE",serviceRS.getString(6));
						 json1.put("SSTATUS",serviceRS.getString(7));
					 ...
					 json.put("Files_List", IncomeMTFilesJSONArray1);
					 
					 userReport1 ="select TERMINAL_ID,SERIAL_NO,TERMINAL_MAKE,MODEL_NO,MAKER_DTTM,DECODE(STATUS,'A','ACTIVE','P','PROCESS','R','RETRIVAL','DEACTIVATION'),STORE_ID FROM TERMINAL_MASTER where  USER_ID="+json.getString("telephone")+"";
								 ...
								 while(serviceRS.next())
									{
									 ...
									 json2.put("TERMINAL_ID",serviceRS.getString(1));
									 json2.put("SERIAL_NO",serviceRS.getString(2));
									 json2.put("TERMINAL_MAKE",serviceRS.getString(3));
									 json2.put("MODEL_NO",serviceRS.getString(4));
									 json2.put("MAKER_DTTM",serviceRS.getString(5));
									 json2.put("STATUS",serviceRS.getString(6));
									 json2.put("TSTORE_ID",serviceRS.getString(7));
							
							 
							 IncomeMTFilesJSONArray2.add(json2);
							
							}
						 servicePstmt.close();
						 serviceRS.close();
						 
						 json.put("Files_List1", IncomeMTFilesJSONArray2);
						 
						 userReport2 ="select * from (select MAC_ADDR, DEVICE_IP, IMEI_NO, SERIAL_NO, VERSION, DEVICE_TYPE,decode(STATUS,'A','Active','Deactivated'),TRANS_DTTM FROM AGENT_MOB_IMEI_DATA where  USER_ID='"+json.getString("telephone")+"' order by TRANS_DTTM desc ) where rownum<=5";
							

							servicePstmt = connection.prepareStatement(userReport2);
							serviceRS = servicePstmt.executeQuery();
							 ...
							 while(serviceRS.next())
								{
								 ...
								 json3.put("MAC_ADDR",serviceRS.getString(1));
								 json3.put("DEVICE_IP",serviceRS.getString(2));
								 json3.put("IMEI_NO",serviceRS.getString(3));
								 json3.put("SERIAL_NO",serviceRS.getString(4));
								 json3.put("VERSION",serviceRS.getString(5));
								 json3.put("DEVICE_TYPE",serviceRS.getString(6));
								 json3.put("MSTATUS",serviceRS.getString(7));
								 json3.put("TRANS_DTTM",serviceRS.getString(8));
						
						 
						 IncomeMTFilesJSONArray3.add(json3);
						
						}
					 servicePstmt.close();
					 serviceRS.close();
					 
					 json.put("Files_List2", IncomeMTFilesJSONArray3);
...
public ResponseDTO AgentMobileStatusAck(RequestDTO requestDTO) {
	Connection connection = null;
	String insQRY = "";
	String ip = null;
	...
	try {
		...
		connection = DBConnector.getConnection();
			...
			insQRY = "{call WALLETACCOUNTSPKG.AGENTMOBCHANNELSTATUS(?,?,?,?,?)}";
...
public ResponseDTO AgentUssdStatusAck(RequestDTO requestDTO) {
	Connection connection = null;
	String insQRY = "";
	String ip = null;
...
insQRY = "{call WALLETACCOUNTSPKG.AGENTUSSDCHANNELSTATUS(?,?,?,?,?)}";
...
public ResponseDTO AgentPosStatusAck(RequestDTO requestDTO) {
	Connection connection = null;
	String insQRY = "";
	String ip = null;
	...
	try {
		...
		connection = DBConnector.getConnection();
			...
			insQRY = "{call WALLETACCOUNTSPKG.AGENTPOSCHANNELSTATUS(?,?,?,?,?)}";
