...
public class CommonLoginDAO {
	...
	private String updateLastLogin(String userid) {
		...
		try {
			lastLogin = "SELECT NVL(to_char(LAST_LOGED_IN , 'DD-MM-YYYY HH24:MI:SS'),' ') FROM USER_LOGIN_CREDENTIALS WHERE LOGIN_USER_ID=?";
			if (con == null)
			...
			pstmt.close();
			pstmt = con
					.prepareStatement("UPDATE USER_LOGIN_CREDENTIALS SET LAST_LOGED_IN=localtimestamp WHERE LOGIN_USER_ID=?");
