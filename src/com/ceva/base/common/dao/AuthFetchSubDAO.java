...
public ResponseDTO newsuperagentAuth(RequestDTO requestDTO) {
	...
	try {
		...
		merchantQry = "select AGENTID,B_O_NAME,SUPERAGENT_NAME,ACCOUNTNUMBER,ACCOUNTNAME,BVN,BANK_NAME,EMAIL,MOBILE,MAKER_ID,MAKER_DTTM,"
				+ "STATUS,ADDRESSLINE,CITY,LOCALGOVERNMENT,COUNTRY,"
				+ "STATE,STREET_NAME,WARD_NAME,DOB,GENDER,ID_TYPE,ID_NUMBER,TELCO_TYPE,NATIONALITY,ACCT_NO,BALANCE,to_char(WAD.DATE_CREATED,'dd-mm-yyyy hh:mi:ss') from SUPER_AGENT_TEMP SAT,WALLET_ACCT_DATA_TEMP WAD where SAT.CUST_ID=WAD.CUST_ID and SAT.REF_NUM=? ";
		...
		if (merchantRS.next()) {
			...
			resultJson.put("AGENTID", merchantRS.getString(1));
			resultJson.put("B_O_NAME", merchantRS.getString(2));
			resultJson.put("SUPERAGENT_NAME", merchantRS.getString(3));
			resultJson.put("ACCOUNTNUMBER", merchantRS.getString(4));
			resultJson.put("ACCOUNTNAME", merchantRS.getString(5));
			resultJson.put("BVN", merchantRS.getString(6));
			resultJson.put("BANK_NAME", merchantRS.getString(7));
			resultJson.put("EMAIL", merchantRS.getString(8));
			resultJson.put("MOBILE", merchantRS.getString(9));
			resultJson.put("MAKER_ID", merchantRS.getString(10));
			resultJson.put("MAKER_DTTM", merchantRS.getString(11));
			resultJson.put("STATUS", merchantRS.getString(12));
			resultJson.put("ADDRESSLINE", merchantRS.getString(13));
			resultJson.put("CITY", merchantRS.getString(14));
			resultJson.put("LOCALGOVERNMENT", merchantRS.getString(15));
			resultJson.put("COUNTRY", merchantRS.getString(16));
			resultJson.put("STATE", merchantRS.getString(17));
			resultJson.put("STREET_NAME", merchantRS.getString(18));
			resultJson.put("WARD_NAME", merchantRS.getString(19));
			resultJson.put("DOB", merchantRS.getString(20));
			resultJson.put("GENDER", merchantRS.getString(21));
			resultJson.put("ID_TYPE", merchantRS.getString(22));
			resultJson.put("ID_NUMBER", merchantRS.getString(23));
			resultJson.put("TELCO_TYPE", merchantRS.getString(24));
			resultJson.put("NATIONALITY", merchantRS.getString(25));
			resultJson.put("ACCT_NO", merchantRS.getString(26));
		    resultJson.put("BALANCE", merchantRS.getString(27));
		    resultJson.put("DATE_CREATED", merchantRS.getString(28));
...
public ResponseDTO TerminalDetailsAuth(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("connection is null [" + (connection == null) + "]");
			...
			if (status.equalsIgnoreCase("D")){
				detQry = "select terminal_id,MODEL_NO,TERMINAL_MAKE,SERIAL_NO,Decode(STATUS,'A','Active','D','Deactived','R','Retrival','P','Not yet Assign'),MAKER_ID,MAKER_DTTM,SUPERAGENT_NAME,AGENTID from SUPER_AGENT_TERMINAL_TEMP where REF_NUM=?";
			}
			else{
				detQry = "select terminal_id,MODEL_NO,TERMINAL_MAKE,SERIAL_NO,Decode(STATUS,'A','Active','D','Deactived','R','Retrival','P','Not yet Assign'),MAKER_ID,MAKER_DTTM,SUPERAGENT_NAME,AGENTID from SUPER_AGENT_TERMINAL_TEMP where REF_NUM=?";
			}
		...
		resultJson.put("MODEL_NO",detRS.getString(2));
			...
			resultJson.put("SUPERAGENT_NAME",detRS.getString(8));
			resultJson.put("AGENTID",detRS.getString(9));
		}
...
public ResponseDTO initialProductAuth(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("connection is null [" + (connection == null) + "]");
			...
			if (status.equalsIgnoreCase("D")){
				detQry = "select PRODUCT_CODE,PRODUCT_NAME,MAKER_ID,MAKER_DT from INITIAL_PRODUCT_TEMP where REF_NUM=?";
			}
			else{
				detQry = "select PRODUCT_CODE,PRODUCT_NAME,MAKER_ID,MAKER_DT from INITIAL_PRODUCT_TEMP where REF_NUM=?";
			}
		...
		if (detRS.next()) {
			resultJson.put("PRODUCT_CODE",detRS.getString(1));
		resultJson.put("PRODUCT_NAME",detRS.getString(2));
			resultJson.put("MAKER_ID",detRS.getString(3));
			resultJson.put("MAKER_DTTM",detRS.getString(4));
		}
...
public ResponseDTO viewUserGroup(RequestDTO requestDTO) {
	...
	try {
		merchantQry = "{call GETUSERGROUPDETAILSCHECKER(?,?,?,?)}";
		responseDTO = new ResponseDTO();
		...
		callableStmt.setString(1, ref_no);
		...
		callableStmt = connection.prepareCall(merchantQry);
		callableStmt.setString(1, ref_no);
		callableStmt.setString(2, "View");
		callableStmt.setString(3, "GROUP_AUTH");
		callableStmt.registerOutParameter(4, OracleTypes.VARCHAR);
...
public ResponseDTO productAuth(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("connection is null [" + (connection == null) + "]");
			...
			logger.debug("Status in Page"+status);
				...
				detQry = "select p.PRD_CODE,p.PRD_NAME,p.PRD_CCY,p.APPLICATION,p.REF_NO,p.TOKEN_VALUE,p.PERDAY_LIMIT,p.CHANNEL_PERDAY_LIMIT,p.FEE_CHARGE " 
	                    +"from product_temp p  where p.REF_NO=?";
		...
		if (detRS.next()) {
			resultJson.put("productCode",  detRS.getString(1));
			resultJson.put("productDesc",  detRS.getString(2));
			resultJson.put("binCurrency",  detRS.getString(3));
			resultJson.put("application",  detRS.getString(4));
			resultJson.put("refno",  detRS.getString(5));
			resultJson.put("tokenval",  detRS.getString(6));
			resultJson.put("perdaylimit",  detRS.getString(7));
			resultJson.put("channellimit",  detRS.getString(8));
			resultJson.put("feename",  detRS.getString(9));
		}
...
public ResponseDTO limitAuth(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("connection is null [" + (connection == null) + "]");
			...
			logger.debug("Status in Page"+status);
				...
				detQry = "SELECT LMT_FEE_CODE,LMT_FEE_DESC,PRODUCT,APPLICATION "
						+ "FROM LIMIT_FEE_MASTER_TEMP WHERE REF_NO=? ";
		...
		if (detRS.next()) {
			resultJson.put("limitCode",detRS.getString(1));
			resultJson.put("limitDesc",detRS.getString(2));
			resultJson.put("product",detRS.getString(3));
			resultJson.put("application",detRS.getString(4));
		}

		detQry1 =  "select LD.CHANNEL||'-'||(select SERVICEDESC from BANK_SERVICE_CODE_MASTER t where t.SERVICECODE=LD.TXN_CODE),"
				+"LD.FREQUENCY,LD.MAX_CNT,LD.MIN_AMT,LD.MAX_AMT"
                +" from LIMIT_FEE_MASTER_TEMP LM , LIMIT_FEE_DETAILS_TEMP LD where LD.REF_NO=? and LM.REF_NO=LD.REF_NO";
...
public ResponseDTO feeAuth(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("connection is null [" + (connection == null) + "]");
			...
			//System.out.println("auth code :: "+requestJSON.getString("auth_code"));
		...
		if((requestJSON.getString("auth_code")).equalsIgnoreCase("FEEACTAUTH")) {
			
			detQry1 =  "select LD.CHANNEL||'-'||(select SERVICEDESC from BANK_SERVICE_CODE_MASTER t where t.SERVICECODE=LD.TXN_CODE) TXNNAME,"
					...
					+ "FRMVAL,decode(LD.CNT_AMT,'A',LD.MAX_AMT,LD.MAX_CNT) TOVAL,LD.AGENT,LD.CEVA,LD.BANK,THIRD_PARTY,VAT from LIMIT_FEE_MASTER LM , LIMIT_FEE_DETAILS LD where "
					+ " LM.LMT_FEE_CODE=? and LM.REF_NUM=LD.REF_NUM";
			
			detPstmt1 = connection.prepareStatement(detQry1);
			detPstmt1.setString(1, resultJson.getString("limitCode"));
		...
		detQry1 =  "select LD.CHANNEL||'-'||(select SERVICEDESC from BANK_SERVICE_CODE_MASTER t where t.SERVICECODE=LD.TXN_CODE) TXNNAME,"
				+ "LD.FREQUENCY,decode(LD.FEE_TYPE,'P','Percentile','Flat') "
				+ "FLATPER,LD.VALUE FPVALUE,decode(LD.CNT_AMT,'A','Amount','Count') CRT,decode(LD.CNT_AMT,'A',LD.MIN_AMT,LD.MIN_CNT) "
				+ "FRMVAL,decode(LD.CNT_AMT,'A',LD.MAX_AMT,LD.MAX_CNT) TOVAL,LD.AGENT,LD.CEVA,LD.BANK,THIRD_PARTY,VAT from LIMIT_FEE_MASTER_TEMP LM , LIMIT_FEE_DETAILS_TEMP LD where "
				+ " LD.REF_NO=? and LM.REF_NO=LD.REF_NO";
...
public ResponseDTO viewCategory(RequestDTO requestDTO) {
	...
	ref_no = requestDTO.getRequestJSON().getString("ref_no");
	type = requestDTO.getRequestJSON().getString("auth_code");
	merchantMap = new HashMap<String, Object>();
	resultJson = new JSONObject();
	...
	prepareStmt1 = connection
			.prepareStatement("select CAT_ID,CAT_NAME from PAYBILL_CATE_TBL_TEMP where REF_NUM=?");
	prepareStmt1.setString(1, ref_no);
	merchantRS = prepareStmt1.executeQuery();
	while (merchantRS.next()) {
		resultJson.put("catids", merchantRS.getString(1));
		resultJson.put("groupType", merchantRS.getString(2));
	}
	prepareStmt1.close();
	merchantRS.close();
	
	if(type.equalsIgnoreCase("CATSTATUSAUTH")) {
		prepareStmt1 = connection
				.prepareStatement("select BILLER_ID,BILLER_NAME from PAYBILL_CATE_DT where CAT_ID=?");
		prepareStmt1.setString(1, resultJson.getString("catids"));
		merchantRS = prepareStmt1.executeQuery();
		while (merchantRS.next()) {
			rightsJson.put("id", merchantRS.getString(1));
		...
		prepareStmt1 = connection
				.prepareStatement("select BILLER_ID,BILLER_NAME from PAYBILL_CATE_DT_TEMP where REF_NUM=?");
		prepareStmt1.setString(1, ref_no);
		merchantRS = prepareStmt1.executeQuery();
		while (merchantRS.next()) {
			rightsJson.put("id", merchantRS.getString(1));
...
public ResponseDTO viewSubCategory(RequestDTO requestDTO) {
	...
	prepareStmt1 = connection
			.prepareStatement("select CAT_ID,SUBCAT_NAME from PAYBILL_SUBCATE_TBL_TEMP where REF_NUM=?");
	prepareStmt1.setString(1, ref_no);
	merchantRS = prepareStmt1.executeQuery();
	while (merchantRS.next()) {
		resultJson.put("catids", merchantRS.getString(1));
	...
	merchantRS.close();
		...
		prepareStmt1 = connection
				.prepareStatement("select BILLER_ID,BILLER_NAME||'-'||CHANNEL from PAYBILL_SUBCATE_DT_TEMP where REF_NUM=?");
		prepareStmt1.setString(1, ref_no);
		merchantRS = prepareStmt1.executeQuery();
		while (merchantRS.next()) {
			rightsJson.put("id", merchantRS.getString(1));
...
public ResponseDTO viewBillerCategory(RequestDTO requestDTO) {
	...
	try {
		...
		prepareStmt1 = connection
				.prepareStatement("select CATGRP_CODE,CATGRP_NAME from PAYBILLCATE_GRP_TBL_TEMP where REF_NUM=?");
		prepareStmt1.setString(1, ref_no);
		merchantRS = prepareStmt1.executeQuery();
		while (merchantRS.next()) {
			resultJson.put("catids", merchantRS.getString(1));
		...
		if(type.equalsIgnoreCase("BILLERCATSTATUSAUTH")) {
			prepareStmt1 = connection
			...
			merchantRS.close();
		prepareStmt1 = connection
				.prepareStatement("select CAT_ID,SUBCAT_ID,NAME from PAYBILLCATE_GRP_DT_TEMP where REF_NUM=?");
		prepareStmt1.setString(1, ref_no);
		merchantRS = prepareStmt1.executeQuery();
		while (merchantRS.next()) {
			rightsJson = new JSONObject();
...
public ResponseDTO WalletAgentLinkedAccountRegAuthData(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("connection is null [" + (connection == null) + "]");
			...
			if (status.equalsIgnoreCase("D")){
				detQry = "SELECT CM.CUSTOMER_CODE,CM.FIRST_NAME,CM.ACCOUNT_NO,CM.ACC_BRANCH,CI.MOBILE_NUMBER,to_char(CM.DOB,'dd-mm-yyyy'),"
						+ "CM.SUPER_ADM,CM.EMAILID,CM.REMARKS,CA.INSTITUTE,CM.W_PRD_CODE,W_PRD_DESC,DECODE(CM.STATUS,'B','Block','F','Inactive','A','Active','Deactivation'),"
						+ "MARITAL_STATUS,ID_TYPE,ID_DETAILS,ADDRESS,NATIONALITY,CITY,AREA,R_STATE,RL_LGA,WARD_NAME,COUNTRY,BVN,BANK_NAME,ACCOUNT_NO,BANK_CUST_NAME,CM.MCC_NAME,CM.BUSINESS_NAME "				
	                    + "FROM AGENT_CUSTOMER_MASTER_TEMP CM,AGENT_CONTACT_INFO_TEMP CI,WALLET_ACCT_DATA_TEMP CA "
					 	+ "WHERE CM.ID=CI.CUST_ID and CM.ID=CA.CUST_ID AND CM.ID=CA.CUST_ID "
						+ "and CA.rowid = (select max(rowid) from WALLET_ACCT_DATA_TEMP mt where mt.cust_id=cm.id) AND CM.REF_NUM=?";
			}
		...
		resultJson.put("custcode",detRS.getString(1));
			...
			resultJson.put("prodesc",detRS.getString(12));
			resultJson.put("status",detRS.getString(13));
			resultJson.put("martStatus",detRS.getString(14));
			resultJson.put("idtype",detRS.getString(15));
			resultJson.put("iddetails",detRS.getString(16));
			resultJson.put("address",detRS.getString(17));
			resultJson.put("nationality",detRS.getString(18));
			resultJson.put("city",detRS.getString(19));
			resultJson.put("area",detRS.getString(20));
			resultJson.put("state",detRS.getString(21));
			resultJson.put("lga",detRS.getString(22));
			resultJson.put("wardname",detRS.getString(23));
			resultJson.put("country",detRS.getString(24));
			resultJson.put("bvn",detRS.getString(25));
			resultJson.put("bankname",detRS.getString(26));
			resultJson.put("accountno",detRS.getString(27));
			resultJson.put("bankcustname",detRS.getString(28));
			resultJson.put("mcc",detRS.getString(29));
			resultJson.put("business",detRS.getString(30));
		}
...
public ResponseDTO WalletAgentRegistrationAuthData(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("connection is null [" + (connection == null) + "]");
			...
			if (status.equalsIgnoreCase("D")){
				detQry = "SELECT CM.CUSTOMER_CODE,CM.FIRST_NAME,CM.ACCOUNT_NO,CM.ACC_BRANCH,CI.MOBILE_NUMBER,to_char(CM.DOB,'dd-mm-yyyy'),"
						+ "CM.SUPER_ADM,CM.EMAILID,CM.REMARKS,CA.INSTITUTE,CM.W_PRD_CODE,W_PRD_DESC,DECODE(CM.STATUS,'B','Block','F','Inactive','A','Active','Deactivation'),"
						+ "MARITAL_STATUS,ID_TYPE,ID_DETAILS,ADDRESS,NATIONALITY,CITY,AREA,R_STATE,RL_LGA,WARD_NAME,COUNTRY,BVN,BANK_NAME,ACCOUNT_NO,BANK_CUST_NAME,CM.MCC_NAME,CM.BUSINESS_NAME,NVL(CM.RM_CODE,' ') "				
	                    + "FROM AGENT_CUSTOMER_MASTER_TEMP CM,AGENT_CONTACT_INFO_TEMP CI,WALLET_ACCT_DATA_TEMP CA "
					 	+ "WHERE CM.ID=CI.CUST_ID and CM.ID=CA.CUST_ID AND CM.ID=CA.CUST_ID "
						+ "and CA.rowid = (select max(rowid) from WALLET_ACCT_DATA_TEMP mt where mt.cust_id=cm.id) AND CM.REF_NUM=?";
			}
		...
		resultJson.put("custcode",detRS.getString(1));
			...
			resultJson.put("mcc",detRS.getString(29));
			resultJson.put("business",detRS.getString(30));
			resultJson.put("staffcode",detRS.getString(31));
		}
...
public ResponseDTO CashoutAccountRegAuthData(RequestDTO requestDTO) {
	...
	try {
		...
		logger.debug("connection is null [" + (connection == null) + "]");
			...
			if (status.equalsIgnoreCase("D")){
				detQry = "SELECT CM.CUSTOMER_CODE,CM.FIRST_NAME,CM.ACCOUNT_NO,CM.ACC_BRANCH,CI.MOBILE_NUMBER,to_char(CM.DOB,'dd-mm-yyyy'),"
						+ "CM.SUPER_ADM,CM.EMAILID,CM.REMARKS,CA.INSTITUTE,CM.W_PRD_CODE,W_PRD_DESC,DECODE(CM.STATUS,'B','Block','F','Inactive','A','Active','Deactivation'),"
						+ "MARITAL_STATUS,ID_TYPE,ID_DETAILS,ADDRESS,NATIONALITY,CITY,AREA,R_STATE,RL_LGA,WARD_NAME,COUNTRY,BVN,BEN_NAME,BEN_ACCT,BEN_BANK_NAME,CM.MCC_NAME,CM.BUSINESS_NAME,CM.ID "
						+ "FROM AGENT_CUSTOMER_MASTER CM,AGENT_CONTACT_INFO CI,WALLET_ACCT_DATA CA ,AGENT_CASHOUT_ACT_TBL_TMP ACT  "
						+ "WHERE CM.ID=CI.CUST_ID and CM.ID=CA.CUST_ID AND CI.MOBILE_NUMBER=ACT.USER_ID  "  
						+ "AND ACT.REF_NUM=?";
			}
			else{
				detQry = "SELECT CM.CUSTOMER_CODE,CM.FIRST_NAME,CM.ACCOUNT_NO,CM.ACC_BRANCH,CI.MOBILE_NUMBER,to_char(CM.DOB,'dd-mm-yyyy'),"
		...
		resultJson.put("custcode",detRS.getString(1));
			...
			resultJson.put("mcc",detRS.getString(29));
			resultJson.put("business",detRS.getString(30));
			resultJson.put("id",detRS.getString(31));
		}


		if (status.equalsIgnoreCase("D")){
			detQry1 = "select ACCT_NO,ACCT_NAME,BRANCH_CODE,APP_TYPE,ALIAS_NAME from WALLET_ACCT_DATA where CUST_ID=?";
		}
		else
		{
			detQry1 = "select ACCT_NO,ACCT_NAME,BRANCH_CODE,APP_TYPE,ALIAS_NAME from WALLET_ACCT_DATA where CUST_ID=?";
			...
			detPstmt1 = connection.prepareStatement(detQry1);
			detPstmt1.setString(1, resultJson.getString("id"));
...
public ResponseDTO billerFee(RequestDTO requestDTO) {
	...
	try {
		...
		detQry1 =  "select CAT_NAME,BILLER_NAME,BANK||' '||BANK_FEE_TYPE,AGENT||' '||AGENT_FEE_TYPE,SUPERAGENT||' '||SUPERAGENT_FEE_TYPE,CEVA||' '||CEVA_FEE_TYPE,THIRD_PARTY||' '||TR_FEE_TYPE,VAT||' '||VAT_FEE_TYPE,FEE_CHARGE from PAYBILL_FEE_DETAILS_TEMP where REF_NO=?";
