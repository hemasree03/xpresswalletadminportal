			 ...
			 while(serviceRS.next())
				{
				 ...
				 json.put("STORE_NAME",JsonValueValidator.stripXSS(serviceRS.getString(8)));
			 ...
			 String Actiondesc="";
				  ...
				  serviceRS = servicePstmt.executeQuery();
					 ...
					 if(actiontype.equalsIgnoreCase("AGENTADD")) {
						...
						String insQRY = "{call pGetTerminalID(?)}";
						cstmt = connection.prepareCall(insQRY);
						cstmt.registerOutParameter(1, Types.VARCHAR);
						cstmt.executeQuery();
						
						json.put("terminalid",cstmt.getString(1));
						
						
					 
					/*servicePstmt = connection.prepareStatement("select '2XPR'||TERMINALID_SEQ.nextval from DUAL");
						  serviceRS = servicePstmt.executeQuery();
							 ...
							 servicePstmt = connection.prepareStatement("select MODEL_NO,TERMINAL_MAKE,SERIAL_NO,DECODE(STATUS,'A','Active','D','Deactive','R','Retrieve'),MERCHANT_ACTIVE,NIBBSTID FROM TERMINAL_MASTER where TERMINAL_ID='"+requestJSON.getString("terminalid")+"'");
							  	serviceRS = servicePstmt.executeQuery();
								 ...
								 while(serviceRS.next())
								 {
									 json.put("modelno",serviceRS.getString(1));
										...
										json.put("ismerchant",serviceRS.getString(5));
										json.put("merchantid",serviceRS.getString(6));
								 }
...
public ResponseDTO gtAgentRegistrtionack(RequestDTO requestDTO) {
	...
	try {
		...
		cstmt.setString(7, requestJSON.getString("ismerchant"));
		cstmt.setString(8, requestJSON.getString("merchantid"));
				 ...
				 while(serviceRS.next())
					{
					 ...
					 json1.put("STORE_NAME",JsonValueValidator.stripXSS(serviceRS.getString(1)));
...
public ResponseDTO storeRegistrationAck(RequestDTO requestDTO) {
	...
	try {
		...
		System.out.println("store name :: "+JsonValueValidator.stripXSSRev(requestJSON.getString("storename")));
		...
		cstmt.setString(13, JsonValueValidator.stripXSSRev(requestJSON.getString("storename")));
...
public ResponseDTO terminalManagement(RequestDTO requestDTO) {
	...
	try {
		...
		String srchcriteria=requestJSON.getString("srchcriteria");
			...
			lmtfeeQry = "select TERMINAL_ID,SERIAL_NO,MODEL_NO,TERMINAL_MAKE,STATUS,to_char(MAKER_DTTM,'dd-mm-yyyy hh24:mi:ss'),SUPERAGENT_NAME from SUPER_AGENT_TERMINAL where AGENTID=? order by MAKER_DTTM";
		

		getlmtfeePstmt = connection.prepareStatement(lmtfeeQry);
		getlmtfeePstmt.setString(1, srchcriteria);
		...
		while (getlmtfeeRs.next()) {
			...
			json.put("SUPERAGENT_NAME", getlmtfeeRs.getString(7));
			lmtJsonArray.add(json);
...
public ResponseDTO searchSuperagent(RequestDTO requestDTO) {
	...
	try {
		...
		viewDataMap = new HashMap<String, Object>();
		 ...
		 String Actiondesc="";
				 ...
				 servicePstmt = connection.prepareStatement("select SUPERAGENT_NAME,AGENTID FROM SUPER_AGENT");
				  serviceRS = servicePstmt.executeQuery();
...
public ResponseDTO superAgentAgentsView(RequestDTO requestDTO) {
	...
	try {
		...
		String srchcriteria=requestJSON.getString("srchcriteria");
			...
			lmtfeeQry = "select ID,MOBILE_NUMBER,SUPERAGENT_NAME,FIRST_NAME||' '||MIDDLE_NAME||' '||LAST_NAME,MAKER_DTTM from SUPER_AGENT_AGENTS where SUPERAGENT_NAME=? AND STATUS='P' order by MAKER_DTTM";
		...
		while (getlmtfeeRs.next()) {
			...
			json.put("SUPERAGENT_NAME", getlmtfeeRs.getString(3));
			json.put("NAME", getlmtfeeRs.getString(4));
			json.put("MAKER_DTTM", getlmtfeeRs.getString(5));
		...
		viewDataMap = new HashMap<String, Object>();
		 ...
		 String Actiondesc="";
			  ...
			  serviceRS = servicePstmt.executeQuery();
				 ...
				 servicePstmt = connection.prepareStatement("select SUPERAGENT_NAME,SUPERAGENT_NAME||'-'||AGENTID FROM SUPER_AGENT");
				  serviceRS = servicePstmt.executeQuery();
...
public ResponseDTO terminalCreationAck(RequestDTO requestDTO) {
	...
	try {
		...
		cstmt = connection.prepareCall(insQRY);
		cstmt.setString(1, requestJSON.getString("superagentcode"));
		cstmt.setString(2, requestJSON.getString("superagentid"));
		cstmt.setString(3, requestJSON.getString("terminalid"));
		cstmt.setString(4, requestJSON.getString("terminalmake"));
		cstmt.setString(5, requestJSON.getString("modelnumber"));
		cstmt.setString(6, requestJSON.getString("serialnumber"));			
...
public ResponseDTO terminalCreationView(RequestDTO requestDTO) {
	...
	try {
		...
		String  limitcodeCountQry = "select TERMINAL_ID,SERIAL_NO,MODEL_NO,TERMINAL_MAKE,DECODE(STATUS,'A','Active','D','Deactived','R','Retrival','P','Not yet Assign'),to_char(MAKER_DTTM,'dd-mm-yyyy') FROM SUPER_AGENT_TERMINAL WHERE TERMINAL_ID=? ";
