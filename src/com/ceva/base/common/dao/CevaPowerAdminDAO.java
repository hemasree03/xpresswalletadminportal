...
public class CevaPowerAdminDAO {
	...
	public ResponseDTO modifyGroupDetails(RequestDTO requestDTO) {
		...
		try {
			...
			prepareStmt = connection.prepareStatement("insert into auth_pending ( ref_num,maker_id,maker_dttm,auth_code,status,auth_flag,MAKER_BRCODE,ACTION,ACTION_DETAILS) values('"+ref_num+"','"+makerid+"',sysdate,'MODGRP','P','UM','"+branch_code+"','MAKER','Group Code "+groupId+" Status Update')");
			prepareStmt.executeUpdate();
			prepareStmt.close();

			merchantQry = "insert into  user_groups_temp(GROUP_ID ,GROUP_NAME ,APPL_CODE ,MAKER_ID ,MAKER_DTTM ,ENTITY ,JSON_VAL ,GROUP_TYPE ,USER_LEVEL ,STATUS,AUTH_FLAG,REF_NUM) select GROUP_ID ,GROUP_NAME ,APPL_CODE ,MAKER_ID ,MAKER_DTTM ,ENTITY ,JSON_VAL ,GROUP_TYPE ,USER_LEVEL ,STATUS,'AUP','"+ref_num+"'  from user_groups where group_id=?";
			prepareStmt = connection.prepareStatement(merchantQry);

			prepareStmt.setString(1, groupId.toUpperCase());
			...
			merchantQry = "update user_groups_temp set json_Val=? where group_id=? AND REF_NUM=?";
			prepareStmt = connection.prepareStatement(merchantQry);

			
			prepareStmt.setString(1, jsonVal);
			prepareStmt.setString(2, groupId.toUpperCase());
			prepareStmt.setString(3, ref_num);

			int sizeJsonIns1 = prepareStmt.executeUpdate();
			connection.commit();

			logger.debug("Updated row are  [" + sizeJsonIns1 + "]");
			...
			merchantQry = "insert into USER_LINKED_ACTION_TEMP (ID ,PID ,NAME ,CHECKED ,OPEN ,TITLE ,GROUP_ID,AUTH_FLAG,REF_NUM ) "
					+ "values (?,?,?,?,?,?,?,?,? )";
			connection.setAutoCommit(false);
			prepareStmt = connection.prepareStatement(merchantQry);
			...
			try {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					if (json_data.getString("checked").equalsIgnoreCase("true")) {
						...
						prepareStmt.setString(6, title);
						prepareStmt.setString(7, groupId);
						prepareStmt.setString(8, "AUP");
						prepareStmt.setString(9, ref_num);
						prepareStmt.addBatch();
					}
				}
			...
			auditjson.put("transCode", "modify-group");
			auditjson.put("channel", "WEB");
			auditjson.put("message","Group code "+groupId +" Modified Successfully,Pending for Authorization");
			auditjson.put("ip",ServletActionContext.getRequest().getRemoteAddr());
			auditjson.put("det1", "");
			auditjson.put("det2", "modifyGroupDetails");
			auditjson.put("det3", "");
	...
	public ResponseDTO insertUserGroupDetails(RequestDTO requestDTO) {
		...
		try {
			...
			merchantPstmt = connection.prepareStatement("insert into auth_pending ( ref_num,maker_id,maker_dttm,auth_code,status,auth_flag,MAKER_BRCODE,ACTION,ACTION_DETAILS) values('"+ref_num+"','"+makerid+"',sysdate,'ADDNEWGRP','P','UM','"+branch_code+"','MAKER','Group Code "+groupId+" New Group Creation')");
			merchantPstmt.executeUpdate();
			merchantPstmt.close();

			merchantQry = "insert into USER_GROUPS_TEMP (GROUP_ID ,GROUP_NAME ,APPL_CODE ,MAKER_ID ,MAKER_DTTM ,ENTITY ,JSON_VAL,GROUP_TYPE,USER_LEVEL,AUTH_FLAG,REF_NUM,STATUS) "
					+ "values (?,?,?,?,sysdate,?,?,?,?,?,?,?)";
			//PreparedStatement merchantPstmt = connection.prepareStatement(merchantQry);
			merchantPstmt = connection.prepareStatement(merchantQry);
			...
			merchantPstmt.setString(8, userlevel);
			merchantPstmt.setString(9, "AUP");
			merchantPstmt.setString(10, ref_num);
			merchantPstmt.setString(11, "D");
			...
			try {
				...
				for (int i = 0; i < jArray.length(); i++) {
					...
					if (json_data.getString("checked").equalsIgnoreCase("true")) {
						...
						merchantPstmt.setString(8, "AUP");
						merchantPstmt.setString(9, ref_num);
	...
	public ResponseDTO RejectuserAuthorizationAck(RequestDTO requestDTO) {
		...
		public ResponseDTO StatusGroupDetails(RequestDTO requestDTO) {
			...
			try {
				...
				try {
					...
					//jArray = new org.json.JSONArray(jobj.getString(keyVal));
					jArray = jobj.getJSONArray(keyVal);
				} catch (JSONException e) {
					logger.debug("Exception in parsing Json String ["
							+ e.getMessage() + "]");
				}
				...
				auditQry = "{call AUDITPKG.INSERTAUDIT(?,?,?,?,?,?,?,?)}";
				cstmt = connection.prepareCall(auditQry);
				cstmt.setString(1, "Status-group");
				cstmt.setString(2, makerid);
				cstmt.setString(3, "WEB");
				cstmt.setString(4, " Action Selected By group id "+groupId);
				cstmt.setString(5, ip);
				...
				prepareStmt = connection.prepareStatement("insert into auth_pending ( ref_num,maker_id,maker_dttm,auth_code,status,auth_flag,MAKER_BRCODE,ACTION,ACTION_DETAILS) values('"+ref_num+"','"+makerid+"',sysdate,'ACTDCTGRP','P','UM','"+branch_code+"','MAKER','Group Code "+groupId+" Status Update')");
				prepareStmt.executeUpdate();
				prepareStmt.close();
				...
				JSONObject auditjson = new JSONObject();
				auditjson.put(CevaCommonConstants.MAKER_ID,userID);
				auditjson.put("transCode", "userGrpCreation");
				auditjson.put("channel", "WEB");
				auditjson.put("message","Group code "+groupId +" Status Changed Successfully,Pending for Authorization");
				auditjson.put("ip",ServletActionContext.getRequest().getRemoteAddr());
				auditjson.put("det1", "");
				auditjson.put("det2", "StatusGroupDetails");
				auditjson.put("det3", "");
