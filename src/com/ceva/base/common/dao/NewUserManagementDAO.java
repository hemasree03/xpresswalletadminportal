  ...
  public ResponseDTO fetchUserGroupDetails(RequestDTO requestDTO)
  {
	...
	if(grouptypes.equalsIgnoreCase("ICTSUPPORT")){
  	...
  	if(usertype.equalsIgnoreCase("BADM")){
		...
		merchantQry = "SELECT GROUP_ID,GROUP_NAME,substr(user_level,instr(user_level,'-')+1,LENGTH(user_level)),MAKER_ID,to_char(MAKER_DTTM,'DD-MM-YYYY'),DECODE(STATUS,'A','Active','De-Active') FROM  "
