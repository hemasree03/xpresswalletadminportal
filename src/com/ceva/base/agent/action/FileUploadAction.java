	...
	public JSONObject getResponseJSON() {
		...
		public String fileUploadProcessconf()
		{
			...
			try {
				...
				requestJSON.put("limitcode", limitcode);
				requestJSON.put("statuscode", limitcode.split("-")[0]);
				responseJSON.put("limitdesc", limitcode.split("-")[1]);
				requestJSON.put("filename", filename);
				requestJSON.put("records", records);
				System.out.println("limitcode va;l is "+limitcode);
				System.out.println("limitcode va;l is "+limitcode.split("-")[1]);
				System.out.println("limitcode va;l is "+limitcode.split("-")[0]);
				rb = ResourceBundle.getBundle("auth");
				String filepath=rb.getString("STORE_FILE_PATH");
				...
				logger.debug("responseDTO DTO [" + responseDTO + "]");
		...
		public String fileuploadrequestinfoApprovalAck() {
			 ...
			 try {
			  ...
			  if(filetype.equalsIgnoreCase("DISCARD")) {
				  responseDTO = fudao.fileuploaddiscardApprovalAck(requestDTO); 
			  }
