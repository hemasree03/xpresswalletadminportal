...
import com.ceva.xpress.common.successreversal.services.core.SuccessTransReversalServiceCaller;
import com.ceva.xpress.wallet.settlement.services.core.WalletSettlementServiceCaller;
...
public class SettlementServiceCall {
	...
	public static boolean getTxnCodeActionvalid(String txncode,String Action) {
		...
		if(txncode.contentEquals("WALAGNOTBANK") && Action.contentEquals("DISCARD")) {
			vald=true;
		}
		if(txncode.contentEquals("AGENTFUND") && Action.contentEquals("DISCARD")) {
			vald=true;
		}
		if(txncode.contentEquals("AGN2OWNAC") && Action.contentEquals("DISCARD")) {
			vald=true;
		}
		if(txncode.contentEquals("AGNPAYBILLAIRTIME") && Action.contentEquals("DISCARD")) {
			vald=true;
		}
	...
	public static String walletReversal(String reqtype,String userid,String walletrefno,String servicecode) {
		...
		try {
			...
			request1.put("reqtype", reqtype);
			request1.put("userid", userid);
			request1.put("servicecode", servicecode);			
			request1.put("channel", "WEB");
			request1.put("walletrefno", walletrefno);
			logger.info("[walletReversal] [Request] :: "+request1);
			System.out.println( "Request :::" +request1 );
			response =  SuccessTransReversalServiceCaller.route(request1);
			logger.info("[walletReversal] [Response] :: "+response);
			System.out.println("Response Json :::" + response );
			responsemsg=response.toString();
