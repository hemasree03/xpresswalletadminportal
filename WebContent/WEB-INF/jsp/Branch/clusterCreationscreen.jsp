...
$(document).on('click','a',function(event) {
	...
	<%-- elsif(btn == 'limitmodify')
	{
		$('#linkmode').val("OLD")
		$('#limitCode').val(code);
		//$('#limitCode').val(code); 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/branchModifyDetails.action';
		$("#form1").submit();
			return true; 
	} --%>
	...
	else if(btn == 'limitmodify')
	{
		$('#limitCode').val(code);
		$('#limitCode').val(code); 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/BranchModify.action';
		$("#form1").submit();
			return true; 
	}
			...
			<div class="box-content" id="top-layer-anchor">
				<div>
					<a href="#" class="btn btn-success" id="createProduct"
						title='Branch Creation' data-rel='popover'
						data-content='Branch Creation'
						onClick="createLimitData('clusterCreationConfig')"><i
						class='icon icon-plus icon-white'></i>&nbsp;Branch Creation</a> &nbsp;
				</div>
			</div>
