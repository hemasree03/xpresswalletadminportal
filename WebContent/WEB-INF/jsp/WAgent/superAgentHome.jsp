...
$(document).ready(function() {
	...
	$('#btn-back').click(function(){
		...
		$('#super-agent-creation').live('click', function () {
...
function buildtable(jsonArray,divid)
{
	...
	$.each(jsonArray, function(index,jsonObject){
			...
			htmlString = htmlString + "<td id=STATUS name=STATUS ><div  class='label label-info' >Deactive</div></td>";	
				...
				htmlString = htmlString + "&nbsp&nbsp<a class='btn btn-danger' href='#' id='agentstatus_"+i+"' index="+i+" val="+i+" title='Active OR Deactive' data-rel='tooltip'  disabled> <i class='icon icon-lock icon-white'></i></a>";
				htmlString = htmlString + "&nbsp&nbsp<a class='btn btn-warning' href='#' id='agentmodify_"+i+"' index="+i+" val="+i+" title='Modify' data-rel='tooltip' disabled> <i class='icon icon-edit icon-white'></i></a>";  
/* 				htmlString = htmlString + "&nbsp&nbsp<a class='btn btn-warning' href='#' id='addproduct_"+i+"' index="+i+" val="+i+" title='Add Product' data-rel='tooltip' disabled> <i class='icon icon-plus icon-white'></i></a>";  
 */
...
$(document).on('click','a',function(event) {
	...
	else if(btn == 'agentmodify')
	{
		$('#accNumbers').val(code);
		$('#cmd').val('VIEW');
		
		$('#limitCode').val(code); 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/superagentmodify.action';
		$("#form1").submit();
		}
	...
	else if(btn == 'addproduct')
	{
		...
		$('#limitCode').val(code); 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/superagentaddproduct.action';
		$("#form1").submit();
		}
...
<body>
	...
	<input type="hidden" name="srchcriteria" id="srchcriteria" value="DIRECT"/>
</div>
</form>
<script type="text/javascript">
$(function(){
