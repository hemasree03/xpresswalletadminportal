...
$(document).ready(function () { 
	...
	$.each(json, function(i, v) {
		...
		if(v.STATUS == 'Active') {
			user_status = "<a href='#' class='label label-success' index='"+rowindex+"'>"+v.STATUS+"</a>";
			status_class = "btn btn-danger";
			text = "<i class='icon icon-ok-circle icon-white'></i>";
			text1 = "Inactive";
		} else if(v.STATUS == 'De-Active') {
			user_status = "<a href='#'  class='label label-warning'  index='"+rowindex+"'>"+v.STATUS+"</a>";
		...
		"<td>"+user_status+"</td>"+
		"<td><a class='btn btn-info' id='create-user' href='#' index='"+rowindex+"' title='Create User' data-rel='tooltip' ><i class='icon icon-plus icon-white'></i></a>&nbsp;"+
		"<a class='"+status_class+"' href='#' id='activ-deactiv-group' index='"+rowindex+"' title='"+text1+"' data-rel='tooltip' > "+text+"</a>&nbsp;"+
		"<a id='modify-group'  class='btn btn-warning' href='#' index='"+rowindex+"'  title='Modify Group' data-rel='tooltip' ><i class='icon icon-edit icon-white'></i></a> &nbsp;"+
		"<a id='view-group' class='btn btn-success' index='"+rowindex+"'  href='#' title='View Group' data-rel='tooltip' ><i class='icon icon-book icon-white'></i></a> </td></tr>";
			...
			if(typeOFReq == "USERGROUP"){
				...
				$.getJSON("groupUserJsonAct.action", queryString,function(data) { 
					...
					$.each(json, function(i, v) {
						...
						var lastTd="<p><a class='"+status_class+"' href='#' id='activ-deactiv-user' index='"+rowindex+"' title='"+text1+"' data-rel='tooltip' > "+text+"</a>&nbsp;"+
						"<a  class='btn btn-success' href='#' id='view-user' index='"+rowindex+"' title='View User' data-rel='tooltip'> <i class='icon icon-book icon-white'></i></a>&nbsp;"+
						"<a  class='btn btn-success' href='#' id='password-reset' index='"+rowindex+"' title='Password Reset' data-rel='tooltip'> <i class='icon icon-refresh icon-white'></i></a>&nbsp;"+
						"<a class='btn btn-warning' href='#' id='modify-user' index='"+rowindex+"'  title='Modify User' data-rel='tooltip'><i class='icon icon-edit icon-white'></i></a></p> &nbsp;&nbsp;";
	...
	if(v_id != 'SEARCH_NO') {
		...
		if(disabled_status == undefined) {  
			if( v_id == "group-creation") { 
				...
				} else if(v_id ==  "activ-deactiv-group") { 
					v_action="viewUserGroup";  
