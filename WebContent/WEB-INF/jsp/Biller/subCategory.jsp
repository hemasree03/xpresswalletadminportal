...
var checkedParentStringPosConstruction = '';
var checkedParentStringUssdConstruction = '';
...
function checkPosLink(){
	var parentId = '#postreeDemo > li'; 
	var getBuildStr = constructString(parentId); 
	getBuildStr = getBuildStr.substr(0,getBuildStr.lastIndexOf(",")); 
	$('#userPosRights').val("["+getBuildStr+"]"); 
	checkedParentStringPosConstruction = '{ "user_Pos_details" : [' + getBuildStr+ ']}';
}

function checkUssdLink(){
	var parentId = '#ussdtreeDemo > li'; 
	var getBuildStr = constructString(parentId); 
	getBuildStr = getBuildStr.substr(0,getBuildStr.lastIndexOf(",")); 
	$('#userUssdRights').val("["+getBuildStr+"]"); 
	checkedParentStringUssdConstruction = '{ "user_Ussd_details" : [' + getBuildStr+ ']}';
}
...
$(document).ready(function(){
	...
	if( metcheck == "Modify") {
		...
		$.fn.zTree.init($("#postreeDemo"), setting, jQuery.parseJSON('${responseJSON.json_val.user_details}')); 
		$.fn.zTree.init($("#ussdtreeDemo"), setting, jQuery.parseJSON('${responseJSON.json_val.user_details}')); 
	} 
	...
	$('#btn-submit').live('click',function() { 
		...
		$('#jsonPosVal').val(checkedParentStringPosConstruction);
		$('#keyPosVal').val("user_Pos_details"); 
		checkUssdLink();
		$('#jsonUssdVal').val(checkedParentStringUssdConstruction);
		$('#keyUssdVal').val("user_Ussd_details"); 
		
	
		$("#form1")[0].action="<%=request.getContextPath()%>/<%=appName %>/viewSubCategoryConfirm.action";
		$("#form1").submit();	 
...
$(document).ready(function() {
	...
	if("${responseJSON.status}"=="Active"){
		$("#stastus").html("<div class='label label-success' style='width:40px' >Active</div>");	
	}else{
		$("#stastus").html("<div class='label label-important' style='width:40px' >Deactive</div>");	
	}
	...
	<!-- topbar ends -->
	 ...
	 <div id="content" class="span10"> 
		<div> 
			<ul class="breadcrumb">
			 <li> <a href="home.action">Home</a> <span class="divider"> &gt;&gt; </span> </li>
			  <li> <a href="userGrpCreation.action">Biller Mapping</a> <span class="divider"> &gt;&gt; </span></li>
			  <li><a href="#"> Sub Category Creation</a></li>
			</ul>
		</div>  
		<div class="row-fluid sortable"> 
			<div class="box span12"> 
				<div class="box-header well" data-original-title>
						<i class="icon-edit"></i> Sub Category Creation
					<div class="box-icon">
				...
				<table width="950" border="0" cellpadding="5" cellspacing="1" 
					class="table table-striped table-bordered bootstrap-datatable "> 
						...
						<tr >
							...
							<td width="25%"><label for="Region"><strong>Category Name </strong></label></td>
							<td width="25%">
							 ${responseJSON.groupType}</td>
							 <td  width="25%"><label for="Region"><strong>Category Id </strong></label></td>
							<td  width="25%" >
	  ...
	  <div class="form-actions" id="form-modify-but">
			...
			<input type="button" name="btn-cancel" class="btn btn-success" id="btn-cancel" value="Cancel" />  
	   </div> 
	</div> 
	<input type="hidden" name="groupType" id="groupType" value="${responseJSON.groupType}"/>
	<input type="hidden" name="catids" id="catids" value="${responseJSON.catids}"/>
...
<input type="hidden" name="jsonPosVal"  id="jsonPosVal" value="" />
<input type="hidden" name="jsonUssdVal"  id="jsonUssdVal" value="" />
