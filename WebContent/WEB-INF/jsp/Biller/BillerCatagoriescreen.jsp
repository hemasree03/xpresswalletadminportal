...
/* htmlString = htmlString + "<td id='' ><a class='btn btn-warning' href='#' id='limitmodify' index="+i+" val="+i+" title='View User' data-rel='tooltip' > <i class='icon icon-book icon-white'></i></a></td>";
  */
	...
	htmlString = htmlString + "<td id='' ><a class='btn btn-warning' href='#' id='limitactive' index="+i+" val="+i+" title='Status Active/Deactive' data-rel='tooltip' '> <i class='icon icon-lock icon-white'></i></a>";
...
$(document).on('click','a',function(event) {
	...
	if(btn == 'limitview')
	{
		...
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/viewBillerCategory.action';
		$("#form1").submit();
			return true; 
	}
	...
	else if(btn == 'limitmodify')
	{
		...
		$('#status').val(status);
		$('#type').val("Modify"); 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/modifyBillerCategory.action';
		$("#form1").submit();
			return true; 
	}
	else if(btn == 'limitactive')
	{
		...
		$('#status').val(status);
		$('#type').val("Status"); 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/statusBillerCategory.action';
		$("#form1").submit();
			return true; 
	}
			...
			<div>
				<ul class="breadcrumb">
					<li><a href="home.action">Home</a> <span class="divider">
							&gt;&gt; </span></li>
					<li><a href="#">Biller Category Management</a></li>
				</ul>
			</div>
			...
			<div class="box-content" id="top-layer-anchor">
				<div>
					<a href="#" class="btn btn-success" id="createProduct"
						title='Biller Category Group Creation' data-rel='popover'
						data-content='Biller Category Group Creation'
						onClick="createLimitData('billerCatCreationNew')"><i
						class='icon icon-plus icon-white'></i>&nbsp;Biller Category Group Creation</a> &nbsp;
				</div>
			</div>
