...
function buildtable(jsonArray,divid)
{
	...
	$.each(jsonArray, function(index,jsonObject){
			...
			htmlString = htmlString + "<td id=ACTION_TYPE"+ i + " name=ACTION_TYPE >" + jsonObject.ACTION_TYPE + "</td>";
 			htmlString = htmlString + "<td id=sno name=sno ><a id=sno class='btn btn-info' href='#' onclick=fun('" + jsonObject.PAYMENTREFERENCE + "') title='Agent Request Approval' data-rel='tooltip'><i class='icon icon-book icon-white'></i></a></td>";
...
function fun(v){
	...
	$("#refno").val(v);
	if($("#application").val()=="COUNT_PENDING"){
		
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/unsettlrequestapprovalinfo.action';
		$("#form1").submit();
		return true;
	}
	if($("#application").val()=="COUNT_COMPLETED"){
		
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/unsettlrequestapprovalcomplatedinfo.action';
		$("#form1").submit();
		return true;
	}
...
function redirectAct(){
	$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/unsettlrequestapproval.action';
	$("#form1").submit();
	return true;
}
	...
	<div id="content" class="span10">   
			  ...
			  <div>
				 <ul class="breadcrumb">
					...
					<li><a href="#">Wallet Unsettlement Data Request Approval</a></li>
				</ul>
			</div>
