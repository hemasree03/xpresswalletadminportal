...
$(document).ready(function(){
	...
	var subrules = {
		...
		$('#btn-srch').live('click',function() {  
			...
			$.getJSON("postJson.action", queryString9,function(data) {
				console.log("Priya tesing"+ data.accNumber)
				myApp.hidePleaseWait();
					...
					var queryString = "method=verifyAccount&mnumber="+mobileNumber ;
					
					$.getJSON("postJson.action", queryString,function(data) { 
						if(data.finalCount==0){

							$("#actiontypew").html("<a class='btn btn-warning' href='#' id='accountcreation' onclick=funaa(this.id)  title='Cashout Account Link' data-rel='tooltip' > <i class='icon icon-plus icon-white'></i></a>");
						}else{
							$("#actiontypew").html("<a class='btn btn-warning' href='#' id='accountmodify' onclick=funaa(this.id)  title='Cashout Account Modify' data-rel='tooltip' > <i class='icon icon-edit icon-white'></i></a>");
	...
	$('#btn-submit').click(function(){ 
			...
			if($("#form1").valid()) { 
				myApp.showPleaseWait();
 				$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/cashoutaccountpage.action'; 
				$('#form1').submit();
				 return true; 
			} else { 
				return false;
...
function funaa(v){
	...
	$("#customercode").val($("#customercode").val());
	...
	if($("#form1").valid()) { 
		...
		$.getJSON("postJson.action", queryString,function(data) { 
				...
				if(data.finalCount==0){
					$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/cashoutaccountdetails.action'; 
					$('#form1').submit();
					 return true; 
					
				}
				else{
					$("#billerMsg").html("<font colour='red'> Account Already linked</font>");
					
					 return false; 
				}
		
		});
		}else if(v=="accountmodify"){
			$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/cashoutaccountmodifydetails.action'; 
			$('#form1').submit();
			 return true; 
		}
...
function funaa1(v){
	...
	if($("#form1").valid()) { 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/cashoutaccountview.action'; 
		$('#form1').submit();
		 return true; 
	} else { 
		return false;
...
function funaa2(v){
	...
	if($("#form1").valid()) { 
		...
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/cashoutaccountmodify.action'; 
		$('#form1').submit();
		 return true; 
	} else { 
		return false;
		...
		<div id="content" class="span10">
		    <div>
				<ul class="breadcrumb">
				  ...
				  <li><a href="#">Cashout Account Link</a></li>
				</ul>
			</div>
		<table height="3">
			 <tr>
