...
function buildtable(jsonArray,divid)
{
	...
	$.each(jsonArray, function(index,jsonObject){
			...
			htmlString = htmlString + "<td width='40%' id=CATE_DESCRIPTION name=CATE_DESCRIPTION >" + jsonObject.CATE_DESCRIPTION + "</td>";
			htmlString = htmlString + "<td width='10%' id=MAKER_ID name=MAKER_ID >" + jsonObject.MAKER_ID + "</td>";
			
			
			htmlString = htmlString + "<td id='' width='30%' ><a class='btn btn-warning' href='#' id='mccview' index="+i+" val="+i+" title='View MC Code' data-rel='tooltip' > <i class='icon icon-book icon-white'></i></a>";
			htmlString = htmlString + " <a class='btn btn-warning' href='#' id='limitmodify' index="+i+" val="+i+" title='Edit MC Code' data-rel='tooltip' > <i class='icon icon-edit icon-white'></i></a></td>";
...
$(document).on('click','a',function(event) {
	...
	if(btn == 'limitmodify')
	{
		$('#limitCode').val(code);
		$('#limitCode').val(code); 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/modifyMccAct.action';
		$("#form1").submit();
			return true; 
	}
	if(btn == 'mccview')
	{
		$('#limitCode').val(code);
		$('#limitCode').val(code); 
		$("#form1")[0].action='<%=request.getContextPath()%>/<%=appName %>/mccpopupDetails.action';
		$("#form1").submit();
			return true; 
	}
	...
	<div id="content" class="span10">   
			  ...
			  <div>
				 <ul class="breadcrumb">
					<li><a href="home.action">Home</a> <span class="divider"> &gt;&gt; </span></li>
					 <li><a href="#">Merchant Category Code Management</a></li>
				</ul>
			</div>
			
			 <div class="box-content" id="top-layer-anchor">
				 <div>
					<a href="#" class="btn btn-success" id="createProduct"   title='Merchant Category Code Creation' data-rel='popover'  data-content='Create New Merchant Category Code' onClick="createLimitData('mccCreationConfig')"><i class='icon icon-plus icon-white'></i>&nbsp;Create New Merchant Category Code </a> &nbsp;
				 </div>	
			</div>
           ...
           <div class="row-fluid sortable"><!--/span-->
			...
			<div class="box span12" id="groupInfo">
				<div class="box-header well" data-original-title>Merchant Category Code Information
					<div class="box-icon"> 
